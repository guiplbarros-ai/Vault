...
- [x] Layout base + tema
- [x] Adicionar aba de Cartões de Crédito
...
## 12. Gestão de Cartões de Crédito

### 12.1 Funcionalidades Implementadas

#### Estrutura de Páginas
- **`/cartoes`**: Dashboard principal de cartões
  - Cards de resumo (total em aberto, próximo vencimento, limite disponível, utilização)
  - Grid de cartões com informações de limite e fatura
  - Empty state para quando não há cartões cadastrados

#### Componentes Criados
1. **`cartao-card.tsx`**: Card individual de cartão
   - Exibe informações básicas (nome, últimos dígitos)
   - Barra de progresso de utilização do limite
   - Status visual por cores (verde <50%, amarelo 50-80%, vermelho >80%)
   - Fatura atual e data de vencimento

2. **`cartao-form.tsx`**: Formulário de criação/edição
   - Informações básicas (nome, instituição, bandeira, últimos dígitos)
   - Configurações financeiras (limite, dia fechamento/vencimento)
   - Cálculo automático do melhor dia de compra
   - Campos opcionais (anuidade, taxa de juros)

3. **`fatura-detalhes.tsx`**: Visualização detalhada da fatura
   - Header com status (aberta/fechada/paga/atrasada/parcial)
   - Valores (total, pago, saldo devedor)
   - Datas de fechamento e vencimento
   - Alertas de vencimento próximo ou atraso
   - Lista de transações/lançamentos
   - Botão para registrar pagamento

### 12.2 Modelo de Dados Esperado

**Extensões necessárias no schema:**
```typescript
...
- melhor_dia_compra: number (calculado)
...
### 12.3 Regras de Negócio Implementadas

1. **Cálculo de limite disponível:**
   ```
...
2. **Melhor dia de compra:**
   ...
   - Maximiza prazo entre compra e vencimento

3. **Alertas de vencimento:**
   - Amarelo: 7 dias ou menos antes do vencimento
   - Vermelho: fatura vencida

4. **Status visual de utilização:**
   - Verde: < 50% do limite
   - Amarelo: 50-80% do limite
   - Vermelho: > 80% do limite

### 12.4 Integrações Pendentes

- [ ] Conectar com Supabase (queries para cartões e faturas)
- [ ] Implementar criação/edição de cartões (API)
- [ ] Vincular transações importadas às faturas corretas
- [ ] Implementar registro de pagamento de fatura
- [ ] Criar job para fechamento automático de faturas
- [ ] Implementar detecção de parceladas na importação
- [ ] Adicionar dashboard de análises (padrões de uso, comerciantes recorrentes)

### 12.5 Próximos Passos

1. **Backend/Database:**
   - Criar migrations para tabelas `cartao_credito` e `fatura`
   - Adicionar campos de cartão na tabela `transacao`
   - Implementar RLS policies para cartões e faturas

2. **Services:**
   - Service para cálculo de melhor dia de compra
   - Service para vinculação automática de transação → fatura
   - Service para fechamento automático de fatura
   - Service para detecção de parceladas

3. **UI/UX:**
   - Página de detalhes individual do cartão (drawer ou página)
   - Modal de registro de pagamento
   - Dashboard de análises e insights
   - Visualização de faturas históricas
