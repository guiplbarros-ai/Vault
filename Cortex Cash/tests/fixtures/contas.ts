/**
 * Fixtures - Contas
 * Agent CORE: Implementador
 *
 * Dados de teste para contas
 */

import type { Conta } from '@/lib/types';

export const contas: Conta[] = [
  {
    id: 'conta-corrente',
    instituicao_id: 'inst-banco-brasil',
    nome: 'Conta Corrente',
    tipo: 'corrente',
    saldo_referencia: 1000.00,
    data_referencia: new Date('2025-01-01'),
    saldo_atual: 1000.00,
    ativa: true,
    usuario_id: 'usuario-test',
    created_at: new Date('2025-01-01'),
    updated_at: new Date('2025-01-01'),
  },
  {
    id: 'conta-poupanca',
    instituicao_id: 'inst-banco-brasil',
    nome: 'Poupança',
    tipo: 'poupanca',
    saldo_referencia: 5000.00,
    data_referencia: new Date('2025-01-01'),
    saldo_atual: 5000.00,
    ativa: true,
    usuario_id: 'usuario-test',
    created_at: new Date('2025-01-01'),
    updated_at: new Date('2025-01-01'),
  },
  {
    id: 'conta-investimento',
    instituicao_id: 'inst-xp',
    nome: 'XP Investimentos',
    tipo: 'investimento',
    saldo_referencia: 10000.00,
    data_referencia: new Date('2025-01-01'),
    saldo_atual: 10000.00,
    ativa: true,
    usuario_id: 'usuario-test',
    created_at: new Date('2025-01-01'),
    updated_at: new Date('2025-01-01'),
  },
  {
    id: 'conta-carteira',
    instituicao_id: 'inst-carteira',
    nome: 'Carteira',
    tipo: 'carteira',
    saldo_referencia: 200.00,
    data_referencia: new Date('2025-01-01'),
    saldo_atual: 200.00,
    ativa: true,
    usuario_id: 'usuario-test',
    created_at: new Date('2025-01-01'),
    updated_at: new Date('2025-01-01'),
  },
  {
    id: 'conta-inativa',
    instituicao_id: 'inst-bradesco',
    nome: 'Conta Antiga',
    tipo: 'corrente',
    saldo_referencia: 0,
    data_referencia: new Date('2024-01-01'),
    saldo_atual: 0,
    ativa: false,
    usuario_id: 'usuario-test',
    created_at: new Date('2024-01-01'),
    updated_at: new Date('2024-06-01'),
  },
];

export const contaAtiva = contas[0];
export const contaPoupanca = contas[1];
export const contaInvestimento = contas[2];
export const contaCarteira = contas[3];
export const contaInativa = contas[4];
