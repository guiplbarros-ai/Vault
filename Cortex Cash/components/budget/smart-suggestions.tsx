...
import { Loader2, Lightbulb, TrendingUp, TrendingDown, AlertTriangle, CheckCircle } from 'lucide-react'
...
export function SmartSuggestions({ mesReferencia }: SmartSuggestionsProps) {
  ...
  const loadSuggestions = async () => {
    try {
      ...
      const taxaSaving = totalMesAtual.receita > 0 ? ((totalMesAtual.receita - totalMesAtual.despesa) / totalMesAtual.receita) * 100 : 0
      if (taxaSaving >= 15) {
        suggestionsArray.push({
          type: 'good-control',
