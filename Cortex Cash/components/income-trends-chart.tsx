...
const getChartColors = (): string[] => {
  ...
  return [
    `hsl(${style.getPropertyValue('--chart-8').trim() || '142 71% 45%'})`,
    `hsl(${style.getPropertyValue('--chart-1').trim() || '175 73% 39%'})`,
    `hsl(${style.getPropertyValue('--chart-3').trim() || '171 69% 50%'})`,
    `hsl(${style.getPropertyValue('--chart-5').trim() || '142 76% 36%'})`,
    `hsl(${style.getPropertyValue('--chart-6').trim() || '160 84% 39%'})`,
  ]
}
  ...
  return (
    <Card className="p-6 shadow-md border flex flex-col h-full" style={{
      ...
      <div className="mb-6 flex-shrink-0">
        <h3 className="text-lg font-bold text-white">Evolução de Receitas</h3>
      ...
      {loading ? (
        ...
        <div className="flex items-center justify-center flex-1 min-h-[300px]">
          <p className={isDark ? "text-sm text-white/70" : "text-sm text-muted-foreground"}>Nenhuma receita encontrada</p>
        </div>
      ) : (
