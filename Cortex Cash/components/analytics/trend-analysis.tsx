    ...
    try {
      ...
      const trendList: TrendData[] = Array.from(allCategories)
        .map(categoria => {
          ...
          const variacaoPercentual = previous === 0 ? (current > 0 ? 100 : 0) : (variacao / previous) * 100
  ...
  return (
    <Card className="p-6">
      ...
      <div className="space-y-3">
        {trends.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">Sem dados para comparação</p>
        ) : (
          trends.map((trend, index) => (
            <div key={index} className="flex items-center gap-4 pb-3 border-b last:border-0">
              <div className={`flex-shrink-0 p-2 rounded-lg ${getTrendColor(trend.tendencia)} bg-secondary/50`}>
