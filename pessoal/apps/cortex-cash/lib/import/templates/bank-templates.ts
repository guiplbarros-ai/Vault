...
export function getTemplateByName(
  nome: string
): Omit<TemplateImportacao, 'id' | 'created_at' | 'updated_at'> | undefined {
  return ALL_BANK_TEMPLATES.find((t) => t.nome.toLowerCase().includes(nome.toLowerCase()))
}

/**
 * Helper para buscar templates por instituição
 */
export function getTemplatesByInstitution(
  instituicaoNome: string
): Array<Omit<TemplateImportacao, 'id' | 'created_at' | 'updated_at'>> {
