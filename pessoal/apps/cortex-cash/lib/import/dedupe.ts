...
export async function generateTransactionHash(
  ...
  const dataISO =
    transacao.data instanceof Date ? transacao.data.toISOString().split('T')[0] : transacao.data
  const canonical = [
 ...
 */
export function removeDuplicatesInArray(transacoes: ParsedTransacao[]): ParsedTransacao[] {
 ...
 */
export function calculateDuplicationRate(original: number, afterDedupe: number): number {
 ...
 */
export function groupByHash(transacoes: ParsedTransacao[]): Map<string, ParsedTransacao[]> {
