 ...
 */
export async function parseOFX(content: string, config: ParseConfig = {}): Promise<ParseResult> {
    ...
    for (const stmt of statements) {
      for (const txn of stmt.transactions) {
        ...
        try {
          ...
          // Parse valor
          const valor = Number.parseFloat(txn.TRNAMT)
...
export function validateOFX(content: string): {
  ...
  const hasOFXHeader =
    content.includes('OFXHEADER:') || content.includes('<OFX>') || content.includes('<?xml')
