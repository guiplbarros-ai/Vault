...
import type { Settings, SettingsCategory, SettingsChangeEvent } from '../types/settings'
...
export class SettingsService {
  ...
  async setCategory(
    ...
    value: Partial<Settings[typeof category]>
  ): Promise<void> {
    // Valida categoria
  ...
  async importSettings(json: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Erro ao importar settings: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      )
    }
  }
...
export const subscribeToSettings = (path: string, callback: SubscriberCallback) =>
  settingsService.subscribe(path, callback)
