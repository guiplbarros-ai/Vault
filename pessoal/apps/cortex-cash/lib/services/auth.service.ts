...
export class AuthService {
  ...
  async register(data: RegisterDTO): Promise<Usuario> {
    try {
      ...
      const existingUser = await db.usuarios.where('email').equalsIgnoreCase(data.email).first()
  ...
  async login(data: LoginDTO): Promise<AuthSession> {
    try {
      ...
      const usuario = await db.usuarios.where('email').equalsIgnoreCase(data.email.trim()).first()
