    ...
    if (data.conta_pagamento_id) {
      ...
      if (conta.tipo !== 'corrente') {
        throw new ValidationError(
          'Apenas contas-corrente podem ser usadas para pagamento de faturas'
        )
      }
    }
  ...
  async listFaturas(
    ...
    faturas.sort((a, b) => {
      ...
      if (sortBy === 'mes_referencia') {
        ...
        compareA =
          a.data_vencimento instanceof Date ? a.data_vencimento : new Date(a.data_vencimento)
        compareB =
          b.data_vencimento instanceof Date ? b.data_vencimento : new Date(b.data_vencimento)
        compareA = compareA.getTime()
        compareB = compareB.getTime()
    ...
    if (updateData.data_compra) {
      updateData.data_compra =
        updateData.data_compra instanceof Date
          ? updateData.data_compra
          : new Date(updateData.data_compra)
    }
