...
export const contas = sqliteTable('contas', {
  ...
  instituicao_id: text('instituicao_id')
    .notNull()
    .references(() => instituicoes.id, { onDelete: 'cascade' }),
  nome: text('nome').notNull(),
...
export const transacoes = sqliteTable('transacoes', {
  ...
  classificacao_confirmada: integer('classificacao_confirmada', { mode: 'boolean' })
    .notNull()
    .default(false),
...
export const logs_ia = sqliteTable('logs_ia', {
  ...
  categoria_sugerida_id: text('categoria_sugerida_id').references(() => categorias.id, {
    onDelete: 'set null',
  }),
...
export const cartoes_config = sqliteTable('cartoes_config', {
  ...
  instituicao_id: text('instituicao_id')
    .notNull()
    .references(() => instituicoes.id, { onDelete: 'cascade' }),
  conta_pagamento_id: text('conta_pagamento_id').references(() => contas.id, {
    onDelete: 'set null',
  }),
...
export const faturas = sqliteTable('faturas', {
  ...
  cartao_id: text('cartao_id')
    .notNull()
    .references(() => cartoes_config.id, { onDelete: 'cascade' }),
  ...
  fechada_automaticamente: integer('fechada_automaticamente', { mode: 'boolean' })
    .notNull()
    .default(false),

  // Pagamento
  data_pagamento: integer('data_pagamento', { mode: 'timestamp' }),
  transacao_pagamento_id: text('transacao_pagamento_id').references(() => transacoes.id, {
    onDelete: 'set null',
  }),
...
export const faturas_lancamentos = sqliteTable('faturas_lancamentos', {
  id: text('id')
    .primaryKey()
    .$defaultFn(() => crypto.randomUUID()),
  fatura_id: text('fatura_id')
    .notNull()
    .references(() => faturas.id, { onDelete: 'cascade' }),
  transacao_id: text('transacao_id').references(() => transacoes.id, { onDelete: 'cascade' }),
...
export const orcamentos = sqliteTable('orcamentos', {
  ...
  centro_custo_id: text('centro_custo_id').references(() => centros_custo.id, {
    onDelete: 'cascade',
  }),
