...
async function seedTransacoes(contas: Conta[], categorias: Categoria[]): Promise<Transacao[]> {
  ...
  console.log(
    '[SEED] Categorias disponíveis:',
    categorias.map((c) => `${c.nome} (${c.tipo})`).join(', ')
  )

  const catSalario =
    categorias.find((c) => c.tipo === 'receita' && c.nome.toLowerCase().includes('salário')) ||
    categorias.find((c) => c.tipo === 'receita')
  const catAlimentacao =
    categorias.find((c) => c.tipo === 'despesa' && c.nome.toLowerCase().includes('alimentação')) ||
    categorias.find((c) => c.tipo === 'despesa')
  const catTransporte =
    categorias.find((c) => c.tipo === 'despesa' && c.nome.toLowerCase().includes('transporte')) ||
    categorias.find((c) => c.tipo === 'despesa')
  const catMoradia =
    categorias.find(
      (c) =>
        c.tipo === 'despesa' &&
        (c.nome.toLowerCase().includes('moradia') || c.nome.toLowerCase().includes('casa'))
    ) || categorias.find((c) => c.tipo === 'despesa')
  const catLazer =
    categorias.find((c) => c.tipo === 'despesa' && c.nome.toLowerCase().includes('lazer')) ||
    categorias.find((c) => c.tipo === 'despesa')
  const catSaude =
    categorias.find((c) => c.tipo === 'despesa' && c.nome.toLowerCase().includes('saúde')) ||
    categorias.find((c) => c.tipo === 'despesa')
  const catEducacao =
    categorias.find((c) => c.tipo === 'despesa' && c.nome.toLowerCase().includes('educação')) ||
    categorias.find((c) => c.tipo === 'despesa')
  ...
  if (!catSalario || !catAlimentacao || !catTransporte || !catMoradia) {
    ...
    console.warn(
      '[SEED] As transações podem ficar sem categoria. Execute o seed inicial de categorias primeiro.'
    )
  }
