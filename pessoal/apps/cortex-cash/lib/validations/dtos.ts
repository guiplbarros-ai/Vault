...
export const createContaSchema = z.object({
  ...
  saldo_referencia: z.number().finite('Saldo de referência deve ser um número válido'),
  data_referencia: z.date({ required_error: 'Data de referência é obrigatória' }),
  cor: z
    .string()
    .regex(/^#[0-9A-Fa-f]{6}$/, 'Cor deve estar no formato hexadecimal (#RRGGBB)')
    .optional(),
...
export const createInvestimentoSchema = z.object({
  ...
  valor_aplicado: z
    .number()
    .nonnegative('Valor aplicado não pode ser negativo')
    .finite('Valor aplicado deve ser um número válido'),
  valor_atual: z
    .number()
  ...
  taxa_juros: z
    .number()
    .nonnegative('Taxa de juros não pode ser negativa')
    .finite('Taxa de juros deve ser um número válido')
    .optional(),
  rentabilidade_contratada: z
    .number()
    .nonnegative('Rentabilidade contratada não pode ser negativa')
    .finite('Rentabilidade contratada deve ser um número válido')
    .optional(),
...
export const parseConfigSchema = z.object({
  ...
  pular_linhas: z
    .number()
    .int('Pular linhas deve ser um número inteiro')
    .nonnegative('Pular linhas não pode ser negativo')
    .optional(),
  formato_data: z.string().max(20, 'Formato de data muito longo').optional(),
  separador_decimal: z
    .enum([',', '.'], { errorMap: () => ({ message: 'Separador decimal deve ser "," ou "."' }) })
    .optional(),
})
...
export const mapeamentoColunasSchema = z.object({
  data: z
    .number()
    .int('Índice da coluna data deve ser um número inteiro')
    .nonnegative('Índice da coluna data não pode ser negativo'),
  descricao: z
    .number()
    .int('Índice da coluna descrição deve ser um número inteiro')
    .nonnegative('Índice da coluna descrição não pode ser negativo'),
  valor: z
    .number()
    .int('Índice da coluna valor deve ser um número inteiro')
    .nonnegative('Índice da coluna valor não pode ser negativo'),
  tipo: z
    .number()
    .int('Índice da coluna tipo deve ser um número inteiro')
    .nonnegative('Índice da coluna tipo não pode ser negativo')
    .optional(),
  categoria: z
    .number()
    .int('Índice da coluna categoria deve ser um número inteiro')
    .nonnegative('Índice da coluna categoria não pode ser negativo')
    .optional(),
  observacoes: z
    .number()
    .int('Índice da coluna observações deve ser um número inteiro')
    .nonnegative('Índice da coluna observações não pode ser negativo')
    .optional(),
})
...
export const createTemplateImportacaoSchema = z.object({
  ...
  nome: z.string().min(1, 'Nome é obrigatório').max(100, 'Nome muito longo'),
  tipo_arquivo: z.enum(['csv', 'ofx', 'excel'], {
    errorMap: () => ({ message: 'Tipo de arquivo deve ser "csv", "ofx" ou "excel"' }),
  }),
  separador: z.string().length(1, 'Separador deve ter 1 caractere').optional(),
  encoding: z.string().max(20, 'Encoding muito longo').optional(),
  pular_linhas: z
...
export type CreateHistoricoInvestimentoDTOValidated = z.infer<
  typeof createHistoricoInvestimentoSchema
>
