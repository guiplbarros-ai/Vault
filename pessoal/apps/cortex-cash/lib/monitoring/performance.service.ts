 ...
 */
export function trackPageLoad(path: string, loadTime: number, renderTime: number): PagePerformance {
 ...
 */
export function getPerformanceSummary(startDate?: Date, endDate?: Date): PerformanceSummary {
  ...
  const averagePageLoadTime =
    pages.length > 0 ? pages.reduce((sum, p) => sum + p.loadTime, 0) / pages.length : 0
