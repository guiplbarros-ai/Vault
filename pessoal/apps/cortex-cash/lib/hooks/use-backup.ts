...
import type { BackupData, ExportOptions, ImportOptions, ImportResult } from '@/lib/backup/types'
...
export function useBackup() {
  ...
  const importData = useCallback(async (file: File, options?: ImportOptions) => {
    try {
      ...
      if (!result.success) {
        throw new Error(result.errors.length > 0 ? result.errors.join(', ') : 'Import failed')
      }
