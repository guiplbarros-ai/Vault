  ...
  const classify = async (items: BatchClassifyItem[]): Promise<BatchClassifyResponse | null> => {
    ...
    try {
      ...
      const totalCategorias =
        (categoriasMap.receita?.length || 0) + (categoriasMap.despesa?.length || 0)
      ...
      toast.success(
        `${data.summary.successful} de ${data.summary.total} transações classificadas`,
        {
