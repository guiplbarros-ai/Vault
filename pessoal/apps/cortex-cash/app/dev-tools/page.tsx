...
const DEV_TOOLS: DevTool[] = [
  ...
  },
  {
    title: 'Seed Completo',
    description:
      'Popula o banco com dados de exemplo completos (instituições, contas, categorias, transações)',
    href: '/dev/seed',
    icon: Database,
    category: 'database',
    status: 'stable',
...
function StatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span
      className={`text-xs px-2 py-1 rounded-full border ${styles[status as keyof typeof styles]}`}
    >
      {status}
    </span>
  )
...
export default function DevToolsPage() {
  const toolsByCategory = DEV_TOOLS.reduce(
    (acc, tool) => {
