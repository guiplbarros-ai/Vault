...
import {
  mapContaToFormData,
  mapDBAccountTypeToFormType,
  mapFormDataToCreateConta,
} from '@/lib/adapters'
...
export default function AccountsPage() {
  ...
  const loadData = async () => {
    await Promise.all([loadAccounts(), loadInstituicoes()])
  }
    ...
    accountsList.forEach((account) => {
      if (!account.conta_pai_id && !processed.has(account.id)) {
        ...
        const children = accountsList.filter(
          (acc) => acc.conta_pai_id === account.id && !processed.has(acc.id)
        )
                  ...
                  <CardHeader className={isLinkedAccount ? 'pb-1 pt-2' : 'pb-3'}>
                    <div className="flex items-start justify-between">
                      <div className={`flex items-center ${isLinkedAccount ? 'gap-2' : 'gap-3'}`}>
                        ...
                        <div>
                          <CardTitle
                            className={`${isLinkedAccount ? 'text-sm' : 'text-base'} text-foreground`}
                          >
                            {account.nome}
                          </CardTitle>
                          <CardDescription
                          ...
                          {contaPai && (
                            ...
                            >
                              ...
                              <span>
                                Vinculada a: <span className="font-semibold">{contaPai.nome}</span>
                              </span>
                            </div>
                          )}
                        </div>
                    ...
                    <div className={isLinkedAccount ? 'space-y-1' : 'space-y-3'}>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1">
                          <span>💸</span>
                          <span>
                            {accountStats[account.id]?.transactionCount || 0} movimentações
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-1">
                    ...
                    <p className="text-lg font-semibold text-foreground">
                      {formatCurrency(selectedAccount.saldo_referencia)}
                    </p>
