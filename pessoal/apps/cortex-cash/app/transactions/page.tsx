...
import { mapDBTypeToFormType, mapFormDataToDTO } from '@/lib/adapters'
...
import type { Categoria, Conta, Tag, Transacao } from '@/lib/types'
...
export default function TransactionsPage() {
        ...
        return (
          <div className="flex flex-wrap gap-1">
            {tagArray.map((tagName) => {
              ...
              return <TagBadge key={tagName} label={tagName} cor={tag?.cor} size="sm" />
            })}
          </div>
        )
      },
            ...
            <DropdownMenuItem
              ...
              onClick={async (e) => {
                ...
                try {
                  ...
                  const categorias = await import('@/lib/services/categoria.service').then((m) =>
                    m.categoriaService.listCategorias({ tipo: row.tipo, ativas: true })
                  )
  ...
  const filters = [
    ...
    },
    {
      ...
      label: 'Categoria',
      options: Array.from(
        new Set(transactions.map((t) => t.categoria_id).filter(Boolean) as string[])
      ).map((cat) => ({
        label: cat,
        value: cat,
      })),
              ...
              onChange={(range) => {
                ...
                if (days > 90) {
                  toast.error('Período muito longo', {
                    description: 'Selecione no máximo 90 dias.',
                  })
                  return
                }
                setDateRange(range)
          ...
          <Card className="glass-card-3d rounded-t-none border-t-0">
            <CardContent className="pt-6">
              ...
              <TabsContent value="filters" className="space-y-6 m-0">
                      ...
                      <TransactionForm
                        ...
                        defaultValues={
                          transactionToEdit
                            ? {
                                type:
                                  transactionToEdit.tipo === 'transferencia'
                                    ? 'expense'
                                    : (mapDBTypeToFormType(transactionToEdit.tipo) as
                                        | 'income'
                                        | 'expense'),
                                description: transactionToEdit.descricao,
                                amount: Math.abs(transactionToEdit.valor),
                                date:
                                  transactionToEdit.data instanceof Date
                                    ? transactionToEdit.data
                                    : new Date(transactionToEdit.data),
                                categoryId: transactionToEdit.categoria_id || '',
                                accountId: transactionToEdit.conta_id || '',
                                tags: transactionToEdit.tags
                                  ? typeof transactionToEdit.tags === 'string'
                                    ? JSON.parse(transactionToEdit.tags)
                        ...
                        <SelectContent>
                          ...
                          {categorias
                            .filter(
                              (c) =>
                                c.pai_id &&
                                (selectedCategory === 'all' || c.pai_id === selectedCategory)
                            )
                ...
                <div className="mt-6">
                  <DataTable data={filteredTransactions} columns={columns} searchable={false} />
                </div>
              </TabsContent>
            ...
            {transactionToView && (
              <div className="space-y-4">
                    ...
                    {transactionToView.categoria_id ? (
                      <>
                        ...
                        <span className="text-sm text-foreground">
                          {getCategoriaById(transactionToView.categoria_id)?.nome || '-'}
                        </span>
                      </>
                    ) : (
                ...
                {(() => {
                  ...
                  const tagArray =
                    typeof transactionToView.tags === 'string'
                      ? JSON.parse(transactionToView.tags)
                      : transactionToView.tags
