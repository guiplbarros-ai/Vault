...
import { REGRAS_COMUNS, clearCommonRules, seedCommonRules } from '@/lib/db/seed-rules'
import { AlertCircle, CheckCircle, Loader2, Play, RefreshCw, Trash2 } from 'lucide-react'
...
export default function SeedRulesPage() {
  ...
  const handleClear = async () => {
    if (
      !confirm(
        'Tem certeza que deseja remover TODAS as regras de seed? Esta ação não pode ser desfeita.'
      )
    ) {
      return
    }
  ...
  const handleReset = async () => {
    if (
      !confirm(
        'Tem certeza que deseja resetar? Isso vai remover e recriar todas as regras de seed.'
      )
    ) {
      return
    }
            ...
            <div className="space-y-3">
              {REGRAS_COMUNS.map((regra, index) => (
                ...
                >
                  ...
                  <div className="text-sm text-muted-foreground">→ {regra.categoria_nome}</div>
                </div>
              ))}
            </div>
          </CardContent>
