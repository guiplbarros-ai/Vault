...
import type { CartaoConfig, Categoria, Fatura, FaturaLancamento } from '@/lib/types'
import type { FaturaLancamentoFormData } from '@/lib/validations'
import { format } from 'date-fns'
import { ptBR } from 'date-fns/locale'
import { ArrowLeft, MoreHorizontal, Pencil, PieChart, Plus, Receipt, Trash2 } from 'lucide-react'
...
export default function FaturaDetalhesPage({ params }: PageProps) {
  ...
  const percentualPago =
    fatura && fatura.valor_total > 0 ? ((fatura.valor_pago || 0) / fatura.valor_total) * 100 : 0

  // Agrupar lançamentos por categoria
  const lancamentosPorCategoria = lancamentos.reduce(
    (acc, lanc) => {
              ...
              <Table>
                ...
                <TableBody>
                  {lancamentos.map((lancamento) => {
                    ...
                    return (
                      <TableRow key={lancamento.id} className="border-white/10 hover:bg-white/5">
                        <TableCell className="text-white/90">
                          {format(new Date(lancamento.data_compra), 'dd/MM/yyyy', { locale: ptBR })}
                        </TableCell>
                        <TableCell className="text-white">
                          <div className="flex flex-col">
                            ...
                            {lancamento.moeda_original && lancamento.moeda_original !== 'BRL' && (
                              <span className="text-xs text-white/50">
                                {lancamento.moeda_original} {lancamento.valor_original?.toFixed(2)}{' '}
                                × {lancamento.taxa_cambio?.toFixed(4)}
                              </span>
                            )}
                          </div>
                        </TableCell>
