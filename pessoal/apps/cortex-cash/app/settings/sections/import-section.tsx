...
import { SettingsCard, SettingsSlider, SettingsToggle } from '@/components/settings'
import { Badge } from '@/components/ui/badge'

export function ImportSection() {
  const [autoDetectDuplicates, setAutoDetectDuplicates] = useSetting<boolean>(
    'importClassification.autoDetectDuplicates'
  )
  const [createPendingTransactions, setCreatePendingTransactions] = useSetting<boolean>(
    'importClassification.createPendingTransactions'
  )
  const [autoApplyRules, setAutoApplyRules] = useSetting<boolean>(
    'importClassification.autoApplyRules'
  )
  const [aiSuggestions, setAiSuggestions] = useSetting<boolean>(
    'importClassification.aiSuggestions'
  )
  const [aiConfidenceThreshold, setAiConfidenceThreshold] = useSetting<number>(
    'importClassification.aiConfidenceThreshold'
  )
  const [autoSaveTemplates, setAutoSaveTemplates] = useSetting<boolean>(
    'importClassification.autoSaveTemplates'
  )
  const [skipInvalidLines, setSkipInvalidLines] = useSetting<boolean>(
    'importClassification.skipInvalidLines'
  )

  return (
    <div className="space-y-6">
      <SettingsCard title="Importação" description="Configure como importar transações">
        <SettingsToggle
