...
import { SettingsCard, SettingsSelect, SettingsSlider, SettingsToggle } from '@/components/settings'
import { Badge } from '@/components/ui/badge'
import { useEffect, useState } from 'react'

export function AICostsSection() {
  const [enabled, setEnabled] = useSetting<boolean>('aiCosts.enabled')
  const [defaultModel, setDefaultModel] = useSetting<'gpt-4o-mini' | 'gpt-4o'>(
    'aiCosts.defaultModel'
  )
  ...
  const [strategy, setStrategy] = useSetting<'aggressive' | 'balanced' | 'quality'>(
    'aiCosts.strategy'
  )
  ...
  const [confidenceThreshold, setConfidenceThreshold] = useSetting<number>(
    'aiCosts.confidenceThreshold'
  )
  ...
  return (
    <div className="space-y-6">
      <SettingsCard title="OpenAI API" description="Configure o modelo e recursos de IA">
      ...
      <SettingsCard title="Controle de Custos" description="Gerencie gastos com API da OpenAI">
        <SettingsSlider
          label="Limite mensal"
      ...
      <SettingsCard title="Otimizações" description="Reduza custos sem perder funcionalidades">
        <SettingsToggle
      ...
      >
        ...
        <div className="rounded-lg border p-4 space-y-2 bg-muted/30">
          <p className="text-xs font-medium text-muted-foreground">ℹ️ Como funciona</p>
          <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
            <li>
              Sugestões com confiança ≥ {Math.round((confidenceThreshold || 0.7) * 100)}%{' '}
              {autoApplyOnImport
                ? 'serão aplicadas automaticamente'
                : 'precisarão de confirmação manual'}
            </li>
            <li>Sugestões abaixo do threshold sempre exigem confirmação manual</li>
            <li>Você pode revisar e alterar classificações a qualquer momento</li>
          </ul>
        </div>
      </SettingsCard>
    </div>
