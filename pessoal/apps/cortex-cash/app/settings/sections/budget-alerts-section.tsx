...
export function BudgetAlertsSection() {
  ...
  const [calculationMethod, setCalculationMethod] = useSetting<'cash' | 'accrual'>(
    'budgetAlerts.calculationMethod'
  )
  const [considerTransfers, setConsiderTransfers] = useSetting<boolean>(
    'budgetAlerts.considerTransfers'
  )
  const [autoProjection, setAutoProjection] = useSetting<boolean>('budgetAlerts.autoProjection')
  const [projectionMethod, setProjectionMethod] = useSetting<
    'avg3months' | 'avg6months' | 'lastMonth' | 'manual'
  >('budgetAlerts.projectionMethod')
  const [resetMonthly, setResetMonthly] = useSetting<boolean>('budgetAlerts.resetMonthly')

  return (
    <div className="space-y-6">
      <SettingsCard title="Alertas" description="Configure quando ser notificado sobre orçamentos">
        <SettingsToggle
      ...
      <SettingsCard title="Cálculo" description="Como calcular o uso do orçamento">
        <SettingsSelect
      ...
      <SettingsCard title="Projeções" description="Projeção automática de gastos futuros">
        <SettingsToggle
