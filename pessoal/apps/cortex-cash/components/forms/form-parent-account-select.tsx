...
export const FormParentAccountSelect = React.forwardRef<
  HTMLButtonElement,
  FormParentAccountSelectProps
>(
  (
    { name, label, description, placeholder, accounts, disabled, className, currentAccountId },
    ref
  ) => {
    const {
      control,
    ...
    const availableAccounts = React.useMemo(() => {
      return accounts.filter(
        (account) => account.tipo === 'corrente' && account.id !== currentAccountId && account.ativa
      )
                ...
                >
                  {selectedValue === 'none' || !selectedAccount ? (
                    <span className="text-white/50">
                      {placeholder || 'Nenhuma (conta independente)'}
                    </span>
                  ) : (
                    <div className="flex items-center gap-2">
                      <div
                        className="flex h-6 w-6 items-center justify-center rounded-full"
                        style={{ backgroundColor: selectedAccount.cor || '#3B82F6' }}
                      >
