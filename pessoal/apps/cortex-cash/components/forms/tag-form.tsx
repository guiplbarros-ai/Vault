...
export function TagForm({ tag, onSubmit, onCancel }: TagFormProps) {
  ...
  const [selectedColor, setSelectedColor] = React.useState(tag?.cor || CORES_PREDEFINIDAS[0])
  ...
  return (
    <form onSubmit={handleSubmit(onSubmitForm)} className="space-y-6">
        ...
        <div className="grid grid-cols-8 gap-2">
          {CORES_PREDEFINIDAS.map((cor) => (
            <button
              ...
              className={`w-10 h-10 rounded-md transition-all hover:scale-[1.02] ${
                selectedColor === cor ? 'ring-2 ring-primary ring-offset-2' : ''
              }`}
              style={{
                backgroundColor: cor,
              }}
      ...
      <div className="flex justify-end gap-3 pt-4">
        ...
        <Button type="submit" disabled={isSubmitting} className="bg-primary text-foreground">
