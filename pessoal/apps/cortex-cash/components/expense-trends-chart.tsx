...
import { THEME_COLORS, getChartColors } from '@/lib/constants/colors'
...
export function ExpenseTrendsChart() {
  ...
  const loadChartData = async () => {
    try {
      ...
      console.log(
        '[ExpenseTrendsChart] Top 3 categorias de despesa:',
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        topCategories.forEach((catId) => {
          const categoria =
            catId === 'sem-categoria' ? null : categorias.find((c) => c.id === catId)
          const categoryName = categoria?.nome || 'Sem Categoria'

          const monthCategoryExpenses = transacoes.filter((t) => {
            const transactionDate = t.data instanceof Date ? t.data : new Date(t.data)
            const matchesCategory =
              catId === 'sem-categoria' ? !t.categoria_id : t.categoria_id === catId
      ...
      return (
        ...
        >
          ...
          {payload.map((entry: any, index: number) => (
            <p
              key={index}
              style={{ fontSize: '12px', color: entry.color, marginBottom: '4px', fontWeight: 500 }}
            >
  ...
  return (
    <Card
      className="glass-card-3d p-6 flex flex-col h-full"
      style={{
        minHeight: '420px',
      }}
    >
