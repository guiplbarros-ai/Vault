...
export function PerformanceDashboard() {
  ...
  async function fullReset() {
    const confirmed = window.confirm(
      'Isso vai limpar cache, Service Worker, localStorage e o banco local. Seus dados locais serão removidos. Continuar?'
    )
    if (!confirmed) return
    try {
      // Clear localStorage
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{summary.queries.slow} slow queries</p>
        ...
        {memory && (
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{memory.percentage.toFixed(1)}%</div>
              <p className="text-xs text-muted-foreground">
      ...
      {slowQueries.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Database queries that exceeded performance thresholds</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {slowQueries.map((query) => (
                <div key={query.id} className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {query.recordCount !== undefined && (
                      <p className="text-xs text-muted-foreground">{query.recordCount} records</p>
                    )}
                    <p className="text-xs text-muted-foreground">
      ...
      {slowPages.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Pages that took longer than expected to load</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {slowPages.map((page) => (
                <div key={page.id} className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
      ...
      {summary.queries.slowestQuery && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>The slowest database query in the current period</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-start gap-3 p-3 rounded-lg bg-red-500/10">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h4 className="font-medium text-sm">{summary.queries.slowestQuery.table}</h4>
