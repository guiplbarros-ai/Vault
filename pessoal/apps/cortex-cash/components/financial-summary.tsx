...
import { AlertTriangle, CreditCard, PieChart, TrendingDown, TrendingUp, Wallet } from 'lucide-react'
...
export function FinancialSummary() {
  ...
  const loadFinancialData = async () => {
    try {
      ...
      const budgetsAtRisk = orcamentos.filter(
        (o) => o.status === 'atencao' || o.status === 'excedido'
      ).length
    ...
    return (
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {[...Array(4)].map((_, i) => (
          <Card
            key={i}
            style={{ backgroundColor: 'rgb(15, 23, 42)', borderColor: 'rgb(30, 41, 59)' }}
          >
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card style={{ backgroundColor: 'rgb(15, 23, 42)', borderColor: 'rgb(30, 41, 59)' }}>
        ...
        <CardContent>
          <p className="text-xs text-white/70">
            ...
            {data.activeAccountsCount === 1 ? 'conta ativa' : 'contas ativas'}
          </p>
        </CardContent>
      </Card>
      ...
      <Card style={{ backgroundColor: 'rgb(15, 23, 42)', borderColor: 'rgb(30, 41, 59)' }}>
        ...
        <CardContent>
          <p className="text-xs text-white/70">
            Receitas: {formatCurrency(data.monthlyIncome)} • Despesas:{' '}
      ...
      <Card style={{ backgroundColor: 'rgb(15, 23, 42)', borderColor: 'rgb(30, 41, 59)' }}>
        ...
        <CardContent>
          <div className="space-y-1">
            <p className="text-xs text-white/70">
              {data.budgetPercentage.toFixed(1)}% usado ({formatCurrency(data.budgetUsed)} de{' '}
              {formatCurrency(data.budgetTotal)})
            </p>
            {data.budgetsAtRisk > 0 && (
              <p className="text-xs text-amber-400 flex items-center gap-1">
                <AlertTriangle className="h-3 w-3" />
                {data.budgetsAtRisk} {data.budgetsAtRisk === 1 ? 'orçamento' : 'orçamentos'} em
                risco
              </p>
            )}
          </div>
      ...
      <Card style={{ backgroundColor: 'rgb(15, 23, 42)', borderColor: 'rgb(30, 41, 59)' }}>
        ...
        <CardContent>
          <div className="space-y-1">
            <p className="text-xs text-white/70">
              {data.creditPercentage.toFixed(1)}% usado ({formatCurrency(data.totalCreditUsed)} de{' '}
