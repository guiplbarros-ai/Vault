...
export function PopularTagsWidget() {
  ...
  const loadPopularTags = useCallback(async () => {
    try {
      ...
      console.log(
        `[PopularTagsWidget] Transações com tags: ${transacoesComTags}/${transacoes.length}`
      )
      console.log(
        '[PopularTagsWidget] Tags processadas:',
        Array.from(tagMap.values())
          .map((t) => `${t.tag.nome} (${t.count})`)
          .join(', ')
      )
      ...
      console.log(
        '[PopularTagsWidget] Top 5 tags:',
        tagsArray.map((t) => `${t.tag.nome} (${t.count})`).join(', ')
      )
        ...
        {popularTags.map(({ tag, count, volume }, index) => (
          ...
          >
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-secondary w-5">#{index + 1}</span>
