...
import { checkAIBudgetLimit, getAIUsageSummary } from '@/lib/services/ai-usage.service'
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {(isNearLimit || isOverLimit) && (
            <AlertTriangle
              className={cn('h-4 w-4', isOverLimit ? 'text-destructive' : 'text-[hsl(42_89%_50%)]')}
            />
          )}
        </div>
        <CardDescription>
          ...
          <span
            className={cn(
              ...
              isOverLimit
                ? 'text-destructive'
                : isNearLimit
                  ? 'text-[hsl(42_89%_40%)] dark:text-[hsl(42_89%_60%)]'
