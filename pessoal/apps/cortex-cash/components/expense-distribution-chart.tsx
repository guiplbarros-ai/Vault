...
export function ExpenseDistributionChart() {
  ...
  const loadChartData = async () => {
    try {
      ...
      const monthExpenses = transacoes.filter((t) => {
        ...
        return transactionDate >= monthStart && transactionDate <= monthEnd && t.tipo === 'despesa'
      })
  ...
  const renderCustomLabel = ({
    ...
    let radiusOffset = 25
    if (percentValue < 5)
      radiusOffset = 30 // Fatias pequenas mais afastadas
  ...
  return (
    ...
    >
      <div className="mb-6 flex-shrink-0">
        <h3 className="text-lg font-bold text-foreground">Distribuição de Despesas</h3>
        <p className="text-sm text-secondary">Todas as categorias do mês</p>
      </div>
