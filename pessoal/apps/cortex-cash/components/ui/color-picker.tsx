    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          disabled={disabled}
          className={cn('w-full justify-start text-left font-normal text-white', className)}
          style={{
            color: '#ffffff',
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span style={{ color: '#ffffff' }}>
              {value ? getColorName(value) : 'Selecione uma cor'}
            </span>
          </div>
        </Button>
      </PopoverTrigger>
      ...
      >
        <div className="grid grid-cols-6 gap-2">
          {colors.map((color) => (
            ...
            >
              {value === color && <Check className="h-4 w-4 text-white drop-shadow-md mx-auto" />}
            </button>
          ))}
        </div>
      </PopoverContent>
