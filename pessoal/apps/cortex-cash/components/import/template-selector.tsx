    ...
    <div className="space-y-6">
      ...
      <div>
        {!searchQuery && favoriteTemplates.length > 0 && templates.length > 0 && (
          <h3 className="text-sm font-medium text-gray-300 mb-3">Todos os templates</h3>
        )}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {templates
            .filter((t) => !favoriteTemplates.some((f) => f.id === t.id) || searchQuery)
            .map((template) => (
              <TemplateCard
                key={template.id}
                template={template}
    ...
    >
      ...
      <div className="space-y-2">
        ...
        {template.contador_uso > 0 && (
          <p className="text-xs text-gray-400">Usado {template.contador_uso}x</p>
        )}
      </div>
    </div>
  )
