      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {loading && <span className="text-xs text-white/60 ml-2">{Math.round(progress)}%</span>}
          </div>
        ...
        {loading && (
          <div className="space-y-1">
            ...
            <p className="text-xs text-white/60 text-right">{Math.round(progress)}% completo</p>
          </div>
        )}
          ...
          >
            ...
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {results.details.map((detail) => (
                ...
                >
                  <span className="text-white/80 truncate flex-1 mr-2">{detail.descricao}</span>
                  {detail.success ? (
                    <Badge variant="outline" className="text-green-300 border-green-500/30 text-xs">
                      {detail.categoria}
                    </Badge>
                  ) : (
                    <span className="text-red-400 text-xs">{detail.error || 'Falhou'}</span>
                  )}
                </div>
              ))}
            </div>
