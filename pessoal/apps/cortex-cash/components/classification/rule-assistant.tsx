...
export function RuleAssistant() {
  ...
  const extractPattern = (descricao: string): string => {
    ...
    let candidate = upperTokens.filter((t) => t.length >= 3).sort((a, b) => b.length - a.length)[0]
      ...
      <CardContent>
          ...
          <div className="space-y-2">
            ...
            <Select value={tipo} onValueChange={(v: TipoTransacaoForm) => setTipo(v)}>
              ...
              <SelectContent className="bg-card border-border">
                <SelectItem value="despesa" className="text-foreground">
                  Despesa
                </SelectItem>
                <SelectItem value="receita" className="text-foreground">
                  Receita
                </SelectItem>
              </SelectContent>
            </Select>
        ...
        {suggestion && (
          <div className="mt-6 p-4 rounded-lg border border-border bg-muted/50 space-y-3">
            ...
            <div className="pt-2">
              <Button onClick={createRule} className="bg-primary text-foreground">
