...
import { Brain, Check, Loader2, Zap } from 'lucide-react'
    ...
    return (
      <>
        <Button
          ...
          title={
            result
              ? `${result.categoria_nome} (${(result.confianca * 100).toFixed(0)}%)`
              : 'Classificar com IA'
          }
          className={cn('hover:bg-white/10', result && 'text-green-400')}
        >
          {loading ? (
        ...
        <Dialog open={showModal} onOpenChange={setShowModal}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Revise a sugestão da IA antes de aplicar</DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="gap-2">
              <Button variant="outline" onClick={handleReject} disabled={applying}>
                Rejeitar
              </Button>
              <Button onClick={() => result && handleApply(result)} disabled={applying || !result}>
                {applying ? (
                  <>
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        ...
        {result && (
          <Badge
            className={cn('gap-1 cursor-pointer', getConfidenceColor(result.confianca))}
