  ...
  return {
    ...
    async execute(params, ctx) {
      ...
      const hours = Math.max(
        1,
        Math.min(24, Number.parseInt((params.hours || '8').trim(), 10) || 8)
      )
      const days = Math.max(1, Math.min(5, Number.parseInt((params.days || '1').trim(), 10) || 1))
      ...
      if (hourly.length) {
        ...
        for (const h of hourly) {
          ...
          lines.push(
            `- ${time}: ${fmtTempC(h.temperatureC)} — ${desc} — chuva ${fmtPct(h.precipitationProbability)} — UV ${fmtUv(h.uvIndex)}`
          )
        }
      }
