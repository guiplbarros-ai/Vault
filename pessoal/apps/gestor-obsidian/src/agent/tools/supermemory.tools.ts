...
export function createSupermemorySearchTool(): AgentTool {
  return {
    ...
    description:
      'Busca memórias no Supermemory (cloud) e carrega trechos relevantes no contexto interno',
    async execute(params, ctx) {
      ...
      const limit =
        Number.isFinite(limitRaw) && limitRaw > 0 ? Math.max(1, Math.min(20, limitRaw)) : 8
