...
async function resolveDefaultProjectId(input: {
    ...
    async execute(params, ctx) {
      ...
      const task = await todoist.createTask({
        ...
        priority: params.priority
          ? (Number.parseInt(params.priority, 10) as 1 | 2 | 3 | 4)
          : undefined,
      })
...
export function createTodoistTasksForPersonTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      const projectName =
        (params.project || '').trim() ||
        (workspaceId === 'freelaw' ? 'Gestão Financeira - Freelaw' : '')
      if (!projectName) {
      ...
      if (assigned.length === 0)
        return `Nenhuma tarefa encontrada para ${collaborator.name} no projeto "${projectName}" (${filter}).`
...
export function createUpdateTaskTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      if (params.priority) updates.priority = Number.parseInt(params.priority, 10) as 1 | 2 | 3 | 4
      if (params.labels) {
      ...
      if (params.project) {
        ...
        const defaultPid = await resolveDefaultProjectId({
          workspaceId: ws,
          content: params.content,
        })
