...
class GoogleDriveService {
  ...
  async listFiles(opts: DriveListOptions = {}): Promise<DriveListResult> {
    ...
    return await this.requestJson<DriveListResult>(
      '/files',
      {},
      {
        q,
  ...
  async getFile(
    fileId: string,
    fields = 'id,name,mimeType,parents,trashed,webViewLink,webContentLink'
  ): Promise<DriveFile> {
    return await this.requestJson<DriveFile>(
  ...
  async move(
    fileId: string,
    input: { addParents: string; removeParents?: string }
  ): Promise<DriveFile> {
    return await this.requestJson<DriveFile>(
      `/files/${encodeURIComponent(fileId)}`,
      {
