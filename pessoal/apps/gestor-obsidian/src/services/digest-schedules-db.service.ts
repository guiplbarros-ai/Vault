...
class DigestSchedulesDbService {
  ...
  async listEnabled(): Promise<DigestScheduleRow[]> {
    ...
    const { data, error } = await supabase.from('digest_schedules').select('*').eq('enabled', true)
    if (error) throw new Error(`Supabase digest_schedules select failed: ${error.message}`)
    return (data || []) as DigestScheduleRow[]
  }

  async replaceForChatKind(input: {
    chatId: number
    kind: DigestKind
    schedules: Array<{ cronExpression: string; timezone?: string }>
  }): Promise<void> {
    const supabase = getSupabaseClient()
