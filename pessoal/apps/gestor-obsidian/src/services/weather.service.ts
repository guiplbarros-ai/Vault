...
class WeatherService {
  ...
  enabled(): boolean {
    return (
      (process.env.TOMORROW_API_KEY || '').trim().length > 0 || DEFAULT_TOMORROW_API_KEY.length > 0
    )
  }
  ...
  async getForecast(input?: {
    ...
    const ttl = Number.isFinite(Number(input?.cacheTtlMs))
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          `Tomorrow.io error: ${res.status} - ${(text || res.statusText).slice(0, 250)}`
        )
      }
