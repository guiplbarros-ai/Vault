...
class GmailService {
  ...
  async listMessages(
    ...
    const { maxResults = 20, q, labelIds, includeSpamTrash = false, pageToken } = options
   ...
   */
  async getUnreadMessages(maxResults = 20): Promise<{ id: string; threadId: string }[]> {
   ...
   */
  async getImportantUnread(maxResults = 10): Promise<{ id: string; threadId: string }[]> {
    return this.listMessages({
      maxResults,
   ...
   */
  async getTodayMessages(maxResults = 50): Promise<{ id: string; threadId: string }[]> {
  ...
  parseMessage(message: GmailMessage): ParsedEmail {
    ...
    return {
      ...
      subject,
      date: dateStr ? new Date(dateStr) : new Date(Number.parseInt(message.internalDate)),
