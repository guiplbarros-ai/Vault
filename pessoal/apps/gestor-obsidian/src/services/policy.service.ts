...
function heuristicDecision(text: string): PolicyDecision {
  ...
  if (explicitSave)
    return { intent: 'save_memory', saveMemory: true, memoryKind: 'note', reason: 'heuristic:save' }
  ...
  const question =
    /\?/.test(text) || /^(o que|como|qual|quais|quem|onde|quando|por que|porque)\b/i.test(t)
...
class PolicyService {
  ...
  private model: string =
    process.env.CORTEX_POLICY_MODEL ||
    process.env.CORTEX_MODEL_FAST ||
  ...
  async apply(
    ...
    prefill: (title: string, payload: string) => void
  ): Promise<void> {
      ...
      try {
        ...
        if (sm.enabled()) {
          ...
          await sm.addMemory({
            ...
            containerTags,
            metadata: {
              source: 'cortex:policy',
              workspaceId: ws,
              chatId,
              kind: decision.memoryKind || 'note',
            },
          })
        }
      } catch {
    ...
    const q =
      (decision.query || '').trim() ||
      (decision.intent === 'lookup_notes' ||
      decision.intent === 'lookup_people' ||
      decision.intent === 'recall'
        ? (decision.query || '').trim()
        : '')
        ...
        if (idx.enabled()) {
          ...
          if (ids.length > 0) {
            ...
            for (const id of ids) {
              ...
              prefill(
                `FONTE: supabase.notes/${note.id} (via Supermemory)`,
