...
class VaultService {
  ...
  vaultExists(): boolean {
    try {
      return fs.existsSync(this.vaultPath) && fs.statSync(this.vaultPath).isDirectory()
    } catch {
      return false
    }
  }
  ...
  listFolders(relativePath = ''): string[] {
    const fullPath = path.join(this.vaultPath, relativePath)
    try {
  ...
  listFiles(relativePath: string, extension = '.md'): string[] {
    const fullPath = path.join(this.vaultPath, relativePath)
    try {
