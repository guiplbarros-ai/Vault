...
class CalendarService {
   ...
   */
  async getTodayEvents(calendarId = 'primary'): Promise<GoogleCalendarEvent[]> {
    const now = new Date()
   ...
   */
  async getWeekEvents(calendarId = 'primary'): Promise<GoogleCalendarEvent[]> {
    const now = new Date()
   ...
   */
  async getNextEvent(calendarId = 'primary'): Promise<GoogleCalendarEvent | null> {
  ...
  parseEvent(event: GoogleCalendarEvent): ParsedCalendarEvent {
    ...
    const start = isAllDay ? parseISO(event.start.date!) : parseISO(event.start.dateTime!)

    const end = isAllDay ? parseISO(event.end.date!) : parseISO(event.end.dateTime!)
   ...
   */
  async quickAdd(text: string, calendarId = 'primary'): Promise<GoogleCalendarEvent> {
