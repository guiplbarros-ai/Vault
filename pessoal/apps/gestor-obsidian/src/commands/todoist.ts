...
export function createTodoistCommand(): Command {
  const todoist = new Command('todoist').description('Gerencia tarefas no Todoist')
    ...
    .action(async (content, options) => {
      try {
        ...
        const taskOptions: Parameters<typeof service.createTask>[0] = {
          content,
          priority: Number.parseInt(options.priority) as 1 | 2 | 3 | 4,
        }
        ...
        if (options.project) {
          ...
          if (project) {
            ...
            if (options.assignee) {
              ...
              if (!collab) {
                console.log(
                  `⚠️ Responsável "${options.assignee}" não encontrado no projeto "${project.name}"`
                )
              } else {
