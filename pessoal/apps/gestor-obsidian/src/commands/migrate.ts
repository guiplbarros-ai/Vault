...
export function createMigrateCommand(): Command {
  ...
  migrate
    ...
    .option(
      '--reindex-supermemory',
      'Força reindex no Supermemory para arquivos unchanged (APPLY only)',
      false
    )
    ...
    .action(async (opts: MigrateOptions) => {
      ...
      const defaultWorkspace =
        (String(opts.workspace || 'pessoal').trim() as WorkspaceId) || 'pessoal'
      ...
      if (!dryRun) {
        ...
        try {
          ...
          if (error) {
            ...
            console.error(
              'Dica: confirme que você está usando a key service_role do MESMO projeto (Settings → API).'
            )
            process.exit(1)
          }
        } catch (e) {
      ...
      console.log(
        `📝 Markdown files encontrados: ${files.length} (processando: ${toProcess.length})`
      )
      ...
      for (const rel of toProcess) {
        ...
        if (dryRun) {
          ...
          if (debug) {
            console.log(
              `✅ would import: ${rel} -> ${workspaceId} title="${title}" tags=${tags.slice(0, 6).join(', ')}`
            )
          }
          continue
        }
