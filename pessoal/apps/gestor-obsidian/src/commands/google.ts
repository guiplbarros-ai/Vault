...
async function persistLocalGoogleTokensToSupabase(
  workspaceId: string,
  tokenPath: string
): Promise<{
  ok: boolean
  ...
  if (!accountEmail)
    return { ok: false, reason: 'Não consegui descobrir o email da conta via Gmail profile' }
...
export function createGoogleCommand(): Command {
  const google = new Command('google').description(
    'Gerencia autenticação com Google (Calendar/Gmail)'
  )
    ...
    .action(() => {
      try {
        ...
        if (status.authenticated)
          console.log(`🧾 Scopes OK: ${authService.hasAllRequiredScopes() ? 'sim' : 'não'}`)
        console.log('')
      } catch (error) {
    ...
    .action(async (opts) => {
      try {
        ...
        try {
          ...
          const persisted = await persistLocalGoogleTokensToSupabase(
            wid,
            authService.getTokenPath()
          )
          if (persisted.ok) {
            console.log(
              `✅ Tokens persistidos no Supabase para: ${persisted.accountEmail} (workspace=${wid})`
            )
          } else {
