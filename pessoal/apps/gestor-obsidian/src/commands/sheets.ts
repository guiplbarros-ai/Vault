...
export function createSheetsCommand(): Command {
  const sheets = new Command('sheets').description('Integrações com Google Sheets')
  ...
  sheets
    ...
    .action(async (opts) => {
      try {
        ...
        const spreadsheetId = opts.spreadsheet || process.env.GOOGLE_SHEETS_FINANCE_SPREADSHEET_ID
        const rangeA1 = opts.range || process.env.GOOGLE_SHEETS_FINANCE_RANGE
        const notionDb = opts.notionDb || process.env.NOTION_FINANCE_DATABASE_ID

        if (!spreadsheetId)
          throw new Error(
            'Spreadsheet ID ausente (flag --spreadsheet ou env GOOGLE_SHEETS_FINANCE_SPREADSHEET_ID)'
          )
        if (!rangeA1)
          throw new Error('Range ausente (flag --range ou env GOOGLE_SHEETS_FINANCE_RANGE)')
        ...
        if (idIndex === -1) {
          throw new Error(
            `Coluna de ID não encontrada no header: "${idCol}". Sugestão: crie uma coluna "ID (Sheets)" no Sheets.`
          )
        }
        ...
        if (!opts.dryRun && !notion) {
          throw new Error(
            'NOTION_API_KEY não configurado (necessário para escrever no Notion via CLI)'
          )
        }
        ...
        for (const r of rows) {
          ...
          if (opts.dryRun) {
            console.log(
              `[DRY] upsert ${rowId}: ${rowObj['Item'] || rowObj['Nome'] || '(sem item)'}`
            )
            continue
          }
