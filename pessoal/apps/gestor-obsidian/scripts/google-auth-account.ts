...
const accountEmail = String(process.argv[3] || '')
  .trim()
  .toLowerCase()

if (!workspaceId || !accountEmail) {
  console.error(
    'Uso: node --import tsx scripts/google-auth-account.ts <workspaceId> <accountEmail>'
  )
  console.error(
    'Ex.: node --import tsx scripts/google-auth-account.ts pessoal guiplbarros@gmail.com'
  )
  process.exit(1)
}
