...
describe('E2E: Smoke Tests', () => {
  ...
  describe('Price Benchmark Flow', () => {
    it('should correctly evaluate prices against benchmarks', async () => {
      ...
      expect(benchmark?.avgPrice).toBe(7600)
      expect(benchmark?.goodPrice).toBe(7200)
      expect(benchmark?.greatPrice).toBe(6000)

      // 2. Rate different prices (ida+volta)
      const greatPrice = ratePriceVsBenchmark('CNF', 'NRT', 5500)
      const goodPrice = ratePriceVsBenchmark('CNF', 'NRT', 6500)
      const normalPrice = ratePriceVsBenchmark('CNF', 'NRT', 7400)
