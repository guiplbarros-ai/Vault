...
function defaultFeeds(): Record<Exclude<NewsTopic, 'hn'>, string[]> {
  ...
  const brFinance = 'https://news.google.com/rss/search?q=mercado%20financeiro%20OR%20bolsa%20OR%20selic%20OR%20inflacao%20OR%20dolar&hl=pt-BR&gl=BR&ceid=BR:pt-419';
  const galo = 'https://news.google.com/rss/search?q=Atl%C3%A9tico%20Mineiro%20OR%20Galo&hl=pt-BR&gl=BR&ceid=BR:pt-419';
  const steelers = 'https://news.google.com/rss/search?q=Pittsburgh%20Steelers&hl=en-US&gl=US&ceid=US:en';
...
function defaultExcludeKeywords(): string[] {
  ...
  return [
    'bolsonaro', 'lula', 'congresso', 'senado', 'camara', 'câmara', 'stf', 'supremo',
    'eleicao', 'eleição', 'eleicoes', 'eleições', 'campanha', 'presidente', 'governo',
    'pt ', 'pl ', 'tse', 'ministro', 'ministra',
  ];
}
...
class NewsService {
  ...
  private async fetchText(url: string): Promise<string> {
    ...
    try {
      const res = await fetch(url, {
        method: 'GET',
        headers: { 'user-agent': 'cortex-bot/1.0', accept: 'application/rss+xml, application/atom+xml, application/xml, text/xml, text/html;q=0.9, */*;q=0.8' },
  ...
  private shouldKeepItem(topic: Exclude<NewsTopic, 'hn'>, title: string): boolean {
    ...
    if (includes.length > 0) {
      const t = normalizeText(title);
      const ok = includes.map(normalizeText).filter(Boolean).some(k => t.includes(k));
      if (!ok) return false;
    }
  ...
  async captureToNotes(input: { workspaceId: string; items: NewsItem[] }): Promise<void> {
    ...
    for (const it of input.items) {
      ...
      const sourcePath = it.topic === 'hn'
        ? (it.raw && typeof (it.raw as any).objectID !== 'undefined' ? `hn/${String((it.raw as any).objectID)}` : `hn/url/${it.url}`)
        : it.url;
