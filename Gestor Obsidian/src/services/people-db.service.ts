...
class PeopleDbService {
  ...
  async upsertByExactName(input: { workspaceId: string; name: string; tags?: string[]; notes?: string }): Promise<DbPerson> {
    const supabase = getSupabaseClient();
    ...
    if (existing.data) {
      const { data, error } = await supabase
        ...
        .select('*')
        .single();
      if (error || !data) throw new Error(`Supabase people update failed: ${error?.message || 'unknown'}`);
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .single();
    if (error || !data) throw new Error(`Supabase people insert failed: ${error?.message || 'unknown'}`);
    return data as DbPerson;
  }
}
