...
export class NotionRefreshService {
  enabled(): boolean {
    return !!(process.env.NOTION_API_KEY_PESSOAL || process.env.NOTION_API_KEY_FREELAW || process.env.NOTION_API_KEY);
  }
  ...
  async refreshWorkspace(ws: WorkspaceId): Promise<{ ingested: number; databases: number }> {
    ...
    for (const targetId of targetIds) {
      ...
      try {
        for (let page = 0; page < 20; page++) {
          ...
          if (lastEdited) {
            ...
            body.filter = { timestamp: 'last_edited_time', last_edited_time: { on_or_after: lastEdited } };
          }

          const res = (await db.queryDatabase(targetId, body)) as unknown as NotionDatabaseQueryResponse;
          ...
          for (const r of rows) {
            try {
              ...
              logger.warn(`Notion refresh: falha ao ingerir page (target=${targetId} ws=${ws}): ${msg}`);
            }
          }
        ...
        await stateDb.upsert({ workspaceId: ws, databaseId: targetId, lastEditedTime: maxEdited || lastEdited });
        continue;
      } catch (e) {
      ...
      try {
        ...
        logger.warn(`Notion refresh: target inválido/sem acesso (ws=${ws} target=${targetId}): ${msg}`);
      }
    }
