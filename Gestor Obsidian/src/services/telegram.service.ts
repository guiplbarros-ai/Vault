...
import { getChatSettingsDbService, type WorkspaceId } from './chat-settings-db.service.js';
...
class TelegramService {
  ...
  private setupDailyDigest(): void {
    ...
    // Inicia os jobs
    void digest.startJobs().catch((e) => logger.error(`Daily Digest startJobs error: ${e instanceof Error ? e.message : String(e)}`));
  }
  ...
  private setupMemoryRefresh(): void {
    ...
    void refresh.startJobs().catch((e) => logger.error(`Memory Refresh startJobs error: ${e instanceof Error ? e.message : String(e)}`));
  }
...
Envie *áudio/voz*, *foto* ou *vídeo* no chat e eu vou registrar e salvar.
    ...
    this.bot.onText(/\/regras\b/, async (msg) => {
      ...
      try {
        ...
        const preview = active.body_md.length > 1800 ? `${active.body_md.slice(0, 1800)}\n\n[…truncado]` : active.body_md;
        await this.sendLongMessage(msg.chat.id, `📌 *Regras ativas* (v${active.version})\n\n${preview}`);
      } catch (e) {
        await this.bot.sendMessage(msg.chat.id, `❌ Erro ao ler regras: ${e instanceof Error ? e.message : String(e)}`);
      }
    });
    ...
    this.bot.onText(/\/aplicar\b/, async (msg) => {
      ...
      if (!pending) {
        await this.bot.sendMessage(chatId, 'ℹ️ Não há nenhuma alteração pendente. Use /config e mande uma regra.');
        return;
      }
      try {
        const db = getRulesDbService();
        ...
        await this.bot.sendMessage(chatId, `❌ Erro ao aplicar regras: ${e instanceof Error ? e.message : String(e)}`);
      }
    });
    ...
    this.bot.onText(/\/todoist_regras\b/, async (msg) => {
      ...
      try {
        ...
        if (base.includes(marker)) {
          await this.bot.sendMessage(msg.chat.id, `ℹ️ As regras do Todoist já estão aplicadas no contexto *${ws}*.`);
          return;
        }
...
- Para “registrar contexto/briefing”, usar comentário com bullets objetivos.
        ...
        await this.bot.sendMessage(msg.chat.id, `✅ Regras do Todoist aplicadas no contexto *${ws}* (v${draft.version}).`, { parse_mode: 'Markdown' });
      } catch (e) {
        await this.bot.sendMessage(msg.chat.id, `❌ Erro ao aplicar regras do Todoist: ${e instanceof Error ? e.message : String(e)}`);
      }
    });
...
Manda ver! 🚀
    ...
    this.bot.onText(/\/areas\b/, async (msg) => {
      ...
      try {
        ...
        if (!taxons.enabled()) {
          await this.bot.sendMessage(msg.chat.id, '❌ Supabase não configurado para taxonomia ainda.');
          return;
        }
        ...
        await this.bot.sendMessage(msg.chat.id, `❌ Erro ao listar áreas: ${e instanceof Error ? e.message : String(e)}`);
      }
    });

    // /area <slug> [notes/<id>] - ajusta tag de área da última nota do chat (ou nota específica)
    this.bot.onText(/\/area(?:\s+([a-z0-9\-]+))?(?:\s+(notes\/[0-9a-fA-F\-]{36}|[0-9a-fA-F\-]{36}))?/, async (msg, match) => {
      if (!(await this.ensureAuthorized(msg))) return;
      const slug = (match?.[1] || '').trim().toLowerCase();
      const rawId = (match?.[2] || '').trim();
      if (!slug) {
        await this.bot.sendMessage(msg.chat.id, '❌ Use: /area <slug> (ex: /area casa) ou /areas para listar.');
        return;
      }
      ...
      try {
        ...
        const noteId = rawId
          ? (rawId.startsWith('notes/') ? rawId.slice('notes/'.length) : rawId)
          : (this.lastCreatedNoteByChat.get(msg.chat.id) || '');
        if (!noteId) {
          await this.bot.sendMessage(msg.chat.id, 'ℹ️ Não achei a última nota deste chat. Use: /area <slug> notes/<id>.');
          return;
        }
        ...
        if (!note) {
          await this.bot.sendMessage(msg.chat.id, `❌ Nota não encontrada no contexto atual: notes/${noteId}`);
          return;
        }
        ...
        await this.bot.sendMessage(msg.chat.id, `✅ Área definida: area/${slug} em notes/${updated.id}`);
      } catch (e) {
        await this.bot.sendMessage(msg.chat.id, `❌ Erro ao ajustar área: ${e instanceof Error ? e.message : String(e)}`);
      }
    });
    ...
    this.bot.onText(/\/contexto(?:\s+(pessoal|freelaw))?/i, async (msg, match) => {
      ...
      try {
        ...
        if (!chatDb.enabled()) {
          ...
          const current = this.workspaceByChat.get(chatId) || ((process.env.CORTEX_DEFAULT_WORKSPACE as WorkspaceId | undefined) || 'pessoal');
          if (!arg) {
            await this.bot.sendMessage(
              chatId,
        ...
        if (!arg) {
          ...
          await this.bot.sendMessage(chatId, `🧭 Contexto atual: *${s.workspace_id}*`, { parse_mode: 'Markdown' });
          return;
        }
        ...
        this.workspaceByChat.set(chatId, s.workspace_id);
        await this.bot.sendMessage(chatId, `✅ Contexto definido para: *${s.workspace_id}*`, { parse_mode: 'Markdown' });
      } catch (e) {
    ...
    this.bot.onText(/\/eu(?:\s+(.+))?/, async (msg, match) => {
      ...
      if (!args) {
        ...
        const b = p.birthday_day && p.birthday_month ? `${String(p.birthday_day).padStart(2, '0')}/${String(p.birthday_month).padStart(2, '0')}` : '(não definido)';
        await this.bot.sendMessage(
          msg.chat.id,
          `👤 Perfil\n- nome: ${p.display_name}\n- aniversário: ${b}\n- timezone: ${p.timezone}\n- locale: ${p.locale}`
        );
      ...
      if (m) {
        ...
        await this.bot.sendMessage(msg.chat.id, `✅ Aniversário salvo no perfil: ${String(p.birthday_day).padStart(2, '0')}/${String(p.birthday_month).padStart(2, '0')}`);
        return;
      }

      await this.bot.sendMessage(msg.chat.id, '❌ Comando inválido. Use:\n- /eu\n- /eu aniversario DD/MM');
    });
      ...
      try {
        ...
        try {
          ...
          if (ws === 'freelaw') {
            ...
            const casa = ['casa', 'obra', 'reforma', 'mudança', 'mudanca', 'apartamento', 'condomínio', 'condominio', 'manutenção', 'manutencao']
              .some(k => t.includes(k));
    ...
    this.bot.onText(/\/status\b/, async (msg) => {
      ...
      const weeklyLines = weekly.length ? weekly.map(s => `• ${s.cronExpression} (semanal)`).join('\n') : '';
      await this.sendLongMessage(
        msg.chat.id,
      ...
      try {
        // Shortcut: after a digest, allow replying with "1,3,4" to mark numbered emails as read.
        {
          ...
          if (onlyNums) {
            ...
            if (last && Array.isArray(last.refs) && last.refs.length > 0 && Date.now() - last.at < 12 * 60 * 60 * 1000) {
              const nums = raw
                .split(/[,\s]+/)
              ...
              if (picked.length === 0) {
                await this.bot.sendMessage(msg.chat.id, 'Não encontrei esses números no último digest.');
                return;
              }
        ...
        // UX sem barra: permitir "contexto pessoal|freelaw"
        {
          ...
          if (m) {
            ...
            try {
              ...
              await this.bot.sendMessage(msg.chat.id, `✅ Contexto definido para: *${s.workspace_id}*`, { parse_mode: 'Markdown' });
            } catch (e) {
              const err = e instanceof Error ? e.message : String(e);
              this.workspaceByChat.set(msg.chat.id, arg);
              await this.bot.sendMessage(
        ...
        // UX sem barra: conectar google (gera URL OAuth do contexto atual)
        {
          ...
          if (/(^|\b)(conectar google|conecta google|login google|google auth|autorizar google)\b/.test(lower)) {
            const chatDb = getChatSettingsDbService();
            const ws = chatDb.enabled()
              ? (await chatDb.getOrCreate(msg.chat.id)).workspace_id
              : (process.env.CORTEX_DEFAULT_WORKSPACE as WorkspaceId | undefined) || 'pessoal';
            ...
            const stateSecret = (process.env.CORTEX_OAUTH_STATE_SECRET || process.env.TELEGRAM_WEBHOOK_SECRET || '').trim();
            ...
            if (/^http:\/\/localhost(?::\d+)?\/oauth2callback$/i.test(redirect)) {
              ...
              const hint = app ? `https://${app}.fly.dev/oauth2callback` : 'https://<seu-app>.fly.dev/oauth2callback';
              await this.bot.sendMessage(
                msg.chat.id,
            ...
            const state = createOAuthState({ chatId: msg.chat.id, workspaceId: ws, ts: Date.now() }, stateSecret);
        ...
        {
          const raw = msg.text.trim();
          const m = raw.match(/^(?:email|conta)\s+([^\s]+@[^\s]+\.[^\s]+)$/i) || raw.match(/^([^\s]+@[^\s]+\.[^\s]+)$/);
          if (m?.[1]) {
            const email = m[1].toLowerCase();
            const chatDb = getChatSettingsDbService();
            if (!chatDb.enabled()) {
              await this.bot.sendMessage(msg.chat.id, '❌ Supabase não configurado para chat_settings.');
              return;
            }
            await chatDb.setGoogleAccountEmail(msg.chat.id, email);
            await this.bot.sendMessage(msg.chat.id, `✅ Conta Google selecionada para este chat: ${email}`);
            return;
          }
        }
        ...
        if (this.configModeChats.has(msg.chat.id)) {
          const db = getRulesDbService();
          if (!db.enabled()) {
            await this.bot.sendMessage(msg.chat.id, '❌ Supabase não configurado para regras (SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY).');
            return;
          }
          const chatDb = getChatSettingsDbService();
          const ws = chatDb.enabled()
          ...
          const draft = await db.createDraftFromComment({
            comment: msg.text,
            actor: msg.from?.username ? `@${msg.from.username}` : (msg.from?.first_name || 'telegram'),
            workspaceId: ws,
          });
  ...
  private async tryHandleMediaMessage(msg: TelegramBot.Message): Promise<boolean> {
    ...
    const kind: 'voice' | 'audio' | 'photo' | 'video' | null =
      voice ? 'voice' :
      audio ? 'audio' :
    ...
    if (kind === 'voice' && voice) {
      ...
      originalName = ((audio as any).file_name as string | undefined) || `audio-${new Date().toISOString().replace(/[:.]/g, '-')}`;
    ...
    if (isSupabaseConfigured()) {
      ...
      try {
        ...
        try {
          const { data } = await supabase.storage.from(bucket).createSignedUrl(storagePath, 60 * 60 * 24 * 7);
    ...
    try {
      ...
      logger.warn(`Falha ao registrar nota no Supabase: ${e instanceof Error ? e.message : String(e)}`);
    }
