...
import { getNewsService, type NewsItem } from './news.service.js';
...
class DailyDigestService {
  ...
  addChat(chatId: number, hour: number = 7, minute: number = 0): void {
    ...
    this.configs = this.configs.filter(c => !(c.chatId === chatId && (c.kind || 'daily') === 'daily'));
   ...
   */
  enableWeeklyDefault(chatId: number, hour: number = 7, minute: number = 0): void {
    ...
    this.configs = this.configs.filter(c => !(c.chatId === chatId && (c.kind || 'daily') === 'weekly'));
    this.configs.push({ id, chatId, cronExpression, kind: 'weekly', enabled: true });
    this.saveConfigsToFile();
    void this.persistChatKindToDb(chatId, 'weekly').catch(() => {});
    void this.startJobs().catch(() => {});
    logger.info(`Weekly Digest: Ativado para chat ${chatId} - segundas ${hour}:${minute.toString().padStart(2, '0')}`);
  }
  ...
  async startJobs(): Promise<void> {
    ...
    if (!this.initialized) {
      ...
      if (db.enabled()) {
        try {
          ...
          logger.info(`Daily Digest: Config carregada do Supabase (${this.configs.length} schedule(s))`);
        } catch (e) {
          logger.error(`Daily Digest: Falha ao carregar do Supabase (${e instanceof Error ? e.message : 'erro'})`);
    ...
    for (const config of this.configs) {
      ...
      const job = cron.schedule(config.cronExpression, async () => {
        ...
        logger.info(`${kind === 'weekly' ? 'Weekly' : 'Daily'} Digest: Executando para chat ${config.chatId}`);
        if (kind === 'weekly') {
      ...
      this.jobs.push(job);
      logger.info(`${(config.kind || 'daily') === 'weekly' ? 'Weekly' : 'Daily'} Digest: Job agendado - ${config.cronExpression} (America/Sao_Paulo)`);
    }
  }
   ...
   */
  private async listGoogleAccountsByWorkspace(workspaceId: 'pessoal' | 'freelaw'): Promise<string[]> {
  ...
  private async getInboxUnreadCount(workspaceId: 'pessoal' | 'freelaw', accountEmail: string): Promise<number> {
    ...
    const inbox = labels.find(l => (l.id || '').toUpperCase() === 'INBOX' || (l.name || '').toUpperCase() === 'INBOX');
  ...
  private emailHighlightReason(input: { from: string; subject: string }): string | null {
    ...
    const security = ['security', 'alert', 'login', 'senha', 'password', 'verificação', 'verificacao', '2fa', 'suspicious', 'suspeit'];
    if (security.some(k => blob.includes(k))) return 'segurança da conta (vale checar se foi você)';

    const finance = ['pagamento', 'cobran', 'fatura', 'boleto', 'invoice', 'nota fiscal', 'nf', 'reembolso', 'pix', 'transfer', 'concilia', 'extrato'];
    if (finance.some(k => blob.includes(k))) return 'financeiro (pode exigir ação/validação)';

    const access = ['acesso', 'api', 'itau', 'itaú', 'token', 'credencial', 'oauth', 'auth', 'integra'];
    if (access.some(k => blob.includes(k))) return 'integração/acesso (pode destravar rotina/processo)';

    const delivery = ['enviado', 'entregue', 'rastream', 'tracking', 'shipment', 'delivery', 'pedido'];
    if (delivery.some(k => blob.includes(k)) || from.includes('amazon')) return 'entrega/pedido em andamento (acompanhar recebimento)';

    const approvals = ['aprova', 'aprovação', 'aprovacao', 'contrato', 'proposta', 'orçament', 'orcament', 'assinar', 'assinatura'];
  ...
  async generateDailyDigest(chatId: number): Promise<string> {
    ...
    try {
      ...
      const location = prefLoc && prefLoc !== '42.3478,-71.0466' ? prefLoc : weather.defaultLocation();
      sections.push('');
      ...
      if (!weather.enabled()) {
        ...
        const label = this.escapeMd(pref?.label || this.locationDisplayName(location, fc.resolvedName) || fc.requestedLocation);
        const d0 =
          fc.daily.find(d => {
        ...
        if (!d0) {
          ...
          const rain = d0.precipitationProbabilityAvg !== undefined ? `${Math.round(d0.precipitationProbabilityAvg)}%` : '—';
        ...
        if (accounts.length === 0) {
          ...
          const perAccount = await Promise.all(accounts.map(async (acc) => {
            ...
            const items = events.map(e => {
              ...
              const time = parsed.isAllDay ? 'Dia inteiro' : format(toZonedTime(parsed.start, tz), 'HH:mm');
              const suffix = parsed.meetLink ? ' (meet)' : '';
              const self = e.attendees?.find(a => a.self);
              const notConfirmed = !!self && (self.responseStatus === 'needsAction' || self.responseStatus === 'tentative');
              const confirmSuffix = notConfirmed ? ' (não confirmado)' : '';
              const title = this.escapeMd(this.ellipsize(parsed.title || '(sem título)', 90)) + this.escapeMd(confirmSuffix);
        ...
        if (accounts.length === 0) {
          ...
          const perAccount = await Promise.all(accounts.map(async (acc) => {
            ...
            for (const ref of refs) {
              ...
              const why = this.emailHighlightReason({ from: fromRaw, subject: parsed.subject || '' });
              if (!why) continue;
              highlights.push({
          ...
          const totalUnread = perAccount.reduce((sum, x) => sum + (Number.isFinite(Number(x.unreadCount)) ? Number(x.unreadCount) : 0), 0);
    ...
    try {
      ...
      if (tasks.length === 0) {
        sections.push('');
        sections.push(`*✅ Tarefas (${isEveningPreview ? 'amanhã + atrasadas' : 'hoje + atrasadas'})*`);
        ...
        for (const p of prios) {
          const label = p === 4 ? 'Prioridade 1' : p === 3 ? 'Prioridade 2' : p === 2 ? 'Prioridade 3' : 'Prioridade (sem urgência)';
          sections.push(`- *${label}*`);
    ...
    if (emailRefs.length > 0) {
      sections.push('');
      sections.push('_Para marcar emails como lidos, responda **apenas com os números** (ex: "1,3,4"). Eu marco automaticamente._');
    }
    return sections.join('\n');
  }

  getLastDigestEmailRefs(chatId: number): { at: number; refs: Array<{ workspaceId: 'pessoal' | 'freelaw'; account: string; id: string }> } | null {
  ...
  async generateWeeklyDigest(_chatId: number): Promise<string> {
    ...
    for (const ws of ['pessoal', 'freelaw'] as const) {
      ...
      try {
          ...
          const perAccount = await Promise.all(accounts.map(async (acc) => {
            ...
            const items = events.map(e => {
              ...
              const when = p.isAllDay
                ...
                : toZonedTime(p.start, tz).toLocaleString('pt-BR', { timeZone: tz, weekday: 'short', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
        ...
        logger.error(`Weekly Digest Calendar error (${ws}): ${e instanceof Error ? e.message : String(e)}`);
        sections.push('*🗓️ Agenda (próximos 7 dias)*');
        sections.push('_Não foi possível carregar agora._');
      }
      ...
      try {
        ...
        logger.error(`Weekly Digest Gmail error (${ws}): ${e instanceof Error ? e.message : String(e)}`);
        sections.push('');
        sections.push('*📧 Emails (pendências)*');
        sections.push('_Não foi possível carregar agora._');
      }
