...
class WeatherPrefsService {
  ...
  async getDefaultLocation(chatId: number): Promise<{ location: string; label?: string | null } | null> {
  ...
  async setDefaultLocation(chatId: number, input: { location: string; label?: string | null }): Promise<void> {
    ...
    if (chatDb.enabled()) {
      try {
        ...
        logger.error(`WeatherPrefsService: fallback to file (${e instanceof Error ? e.message : String(e)})`);
      }
    }
