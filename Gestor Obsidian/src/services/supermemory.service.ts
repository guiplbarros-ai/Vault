...
class SupermemoryService {
  ...
  constructor() {
    this.baseUrl = (process.env.SUPERMEMORY_BASE_URL || 'https://api.supermemory.ai').trim().replace(/\/+$/, '');
  ...
  async addMemory(input: SupermemoryAddMemoryInput): Promise<{ id?: string; documentId?: string }> {
    ...
    const tryAdd = async (path: '/v3/documents' | '/v3/memories', body: Record<string, unknown>) => {
  ...
  private buildSearchPayload(input: SupermemorySearchInput, mode: 'containerTag' | 'containerTags' | 'none'): Record<string, unknown> {
  ...
  async search(input: SupermemorySearchInput): Promise<{ results: SupermemorySearchResult[]; total?: number }> {
    const containerTag = input.containerTag || (input.containerTags && input.containerTags[0]) || undefined;
    ...
    const second = hasMany ? (containerTag ? 'containerTag' : 'none') : (containerTags?.length ? 'containerTags' : 'none');
    ...
    const tryOne = async (
      ...
      try {
        ...
        const noContainer: SupermemorySearchInput = { ...base, containerTag: undefined, containerTags: undefined };
