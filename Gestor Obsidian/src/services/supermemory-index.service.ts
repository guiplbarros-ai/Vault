...
import { getSupermemoryService, type SupermemorySearchResult } from './supermemory.service.js';
...
class SupermemoryIndexService {
  ...
  async searchSupabaseNoteIds(query: string, workspaceId: string, limit: number = 5): Promise<string[]> {
    if (!this.enabled()) return [];
    const sm = getSupermemoryService();
    const q = (query || '').trim();
    if (!q) return [];
