...
class BrainService {
  ...
  private estimateOpenAiUsd(inputTokens: number, outputTokens: number): number | null {
    ...
    if (!Number.isFinite(inPerM) || !Number.isFinite(outPerM) || inPerM <= 0 || outPerM <= 0) return null;
  ...
  private async logOpenAiUsage(chatId: number, model: string, usage?: { prompt_tokens?: number; completion_tokens?: number }): Promise<void> {
    try {
...
[Mostra o próximo evento com horário e detalhes relevantes]`;
  ...
  private loadPersistedState(chatId: number): ConversationState | null {
    try {
      ...
      const messages = Array.isArray(parsed.messages) ? parsed.messages.slice(-30) as Message[] : [];
      const pendingActions = Array.isArray(parsed.pendingActions) ? parsed.pendingActions as PendingAction[] : [];
      const awaitingConfirmation = Boolean(parsed.awaitingConfirmation);
      return {
        messages,
        pendingActions,
        awaitingConfirmation,
        lastNotionData: typeof parsed.lastNotionData === 'string' ? parsed.lastNotionData : undefined,
        lastPrefetchKey: typeof parsed.lastPrefetchKey === 'string' ? parsed.lastPrefetchKey : undefined,
        lastMemoryKey: typeof parsed.lastMemoryKey === 'string' ? parsed.lastMemoryKey : undefined,
        lastMemoryIngestKey: typeof parsed.lastMemoryIngestKey === 'string' ? parsed.lastMemoryIngestKey : undefined,
        lastAutoRetrieveKey: typeof parsed.lastAutoRetrieveKey === 'string' ? parsed.lastAutoRetrieveKey : undefined,
      };
    } catch {
      return null;
    }
  ...
  async chat(chatId: number, userMessage: string): Promise<BrainResponse> {
    ...
    if (state.awaitingConfirmation && state.pendingActions.length > 0) {
      if (this.isConfirmation(userMessage)) {
        ...
        if (!st2.awaitingConfirmation && st2.pendingActions.length === 0) this.clearPersistedState(chatId);
    ...
    if (/\[EXECUTE:\w+\]/.test(userMessage)) {
      ...
      const resultText = results.map(r => (r.success ? `✅ ${r.summary}` : `❌ ${r.actionType}: ${r.error}`)).join('\n');
      ...
      if (toConfirm.length > 0) {
        ...
        const summary = toConfirm
          .map((a) => `- ${a.actionType} ${Object.keys(a.params).length ? `(${Object.entries(a.params).map(([k, v]) => `${k}: ${String(v).slice(0, 60)}`).join(', ')})` : ''}`.trim())
          .join('\n');
        return {
          message: [
    ...
    try {
      ...
      const hasExecuteTags0 = /\[EXECUTE:\w+\]/.test(finalAssistantMessage) || /\bEXECUTE:\w+\b/.test(finalAssistantMessage);
      ...
      if (!alreadyTried && !hasExecuteTags0 && internal0.length === 0 && this.isStallingAnswer(finalAssistantMessage)) {
      ...
      const assistantMessageWithSources = this.appendSourcesIfEnabled(finalAssistantMessage, (state.lastNotionData || '').trim());
      ...
      if (deliberate && !(/\[EXECUTE:\w+\]/.test(assistantMessage) || /\bEXECUTE:\w+\b/.test(assistantMessage))) {
        ...
        state.messages.pop();
        state.messages.push({ role: 'assistant', content: this.appendSourcesIfEnabled(refined, internalData) });
      }

      // Check if response contains EXECUTE tags (AI wants to execute now)
      const finalAssistantMessage2 = state.messages[state.messages.length - 1]?.content || assistantMessageWithSources;
      const hasExecuteTags = /\[EXECUTE:\w+\]/.test(finalAssistantMessage2) || /\bEXECUTE:\w+\b/.test(finalAssistantMessage2);
      ...
      if (hasExecuteTags) {
        ...
        if (toConfirm.length > 0) {
          ...
          const prefix = results.length
            ? results.map(r => (r.success ? `✅ ${r.summary}` : `❌ ${r.actionType}: ${r.error}`)).join('\n')
            : '';
  ...
  private async executePendingActions(chatId: number): Promise<BrainResponse> {
    ...
    const onlyToolActions = state.pendingActions.length > 0 && state.pendingActions.every(a => a.action !== 'PROPOSAL');
    if (onlyToolActions) {
    ...
    try {
      ...
      if (followupNeeded) {
        ...
        const finalMessage = await this.generateFollowupAnswer(chatId, state, cleanMessage, results);
        state.messages.push({ role: 'assistant', content: finalMessage });
        return { message: finalMessage || 'Pronto! Ações executadas.', needsConfirmation: false, pendingActions: [] };
      }
      const resultText = results.map(r => (r.success ? `✅ ${r.summary}` : `❌ ${r.actionType}: ${r.error}`)).join('\n');
      return { message: (cleanMessage ? `${cleanMessage}\n\n${resultText}` : resultText).trim() || 'Pronto!', needsConfirmation: false, pendingActions: [] };
    } catch (error) {
      logger.error(`Brain execution error: ${error}`);
      return { message: 'Ops, tive um problema ao executar. Pode tentar de novo?', needsConfirmation: false, pendingActions: [] };
    }
  }

  private parseExecuteBlocks(message: string): Array<{ actionType: string; params: Record<string, string> }> {
  ...
  private async prefetchSupermemoryIfEnabled(state: ConversationState, chatId: number, userMessage: string): Promise<void> {
    ...
    try {
      ...
      const isRecall =
        /\b(foco|focos|prioridade|prioridades|meta|metas|objetivo|objetivos|lembra|lembrar|relembra|relembrar)\b/i.test(norm) ||
      ...
      const formatted = res.results
        ...
        .map((r, idx) => {
          ...
          const header = [`#${idx + 1}`, title ? `"${title}"` : '', `${id}${score}`].filter(Boolean).join(' ');
          return `${header}\n${chunks}`.trim();
        })
        .join('\n\n');

      const scopeLabel = primaryTag.startsWith('ws_') ? 'workspace' : 'chat';
      this.appendInternalData(state, `SUPERMEMORY (auto:${scopeLabel}) — "${q.substring(0, 80)}"`, formatted, 6500);
    } catch (e) {
  ...
  private shouldAutoIngestToSupermemory(text: string): boolean {
    ...
    const prefs =
      /(minha|minhas|meu|meus)\s+(preferencia|preferencias|regra|regras|politica|politicas)/i.test(t) ||
  ...
  private inferSupermemoryKind(text: string): string {
    ...
    if (/(preferencia|prefiro|sempre que|nunca|daqui pra frente|a partir de agora)/i.test(t)) return 'preference';
  ...
  private async autoIngestSupermemoryIfEnabled(state: ConversationState, chatId: number, userMessage: string): Promise<void> {
    const sm = getSupermemoryService();
    if (!sm.enabled()) return;
  ...
  private async refineAnswer(chatId: number, state: ConversationState, draft: string): Promise<string> {
   ...
   */
  public prefillContext(chatId: number, title: string, payload: string, limit: number = 6500): void {
    const state = this.getState(chatId);
  ...
  private async prefetchContextIfHelpful(state: ConversationState, userMessage: string): Promise<void> {
    const lower = userMessage.toLowerCase();
    const recallLike =
      /(quais eram|quais era|qual era|o que era|lembra|relembra|o que eu te disse|o que eu falei)/i.test(lower) ||
  ...
  private async answerRecallFromSupermemory(chatId: number, userMessage: string): Promise<string | null> {
    const sm = getSupermemoryService();
    ...
    try {
      ...
      if (debugOn) {
        logger.info(`Supermemory recall direto: primaryTag=${primaryTag} workspaceTag=${workspaceTag || '-'} chatTag=${chatTag}`);
      ...
      return [
        `Pelo que tenho salvo na minha memória:`,
        items.join('\n'),
      ].join('\n');
    } catch {
