...
Retorne APENAS um JSON válido no formato:
{
  ...
  "reasoning": "explicação curta da classificação"
    ...
    try {
      ...
      logger.info(`AI classificou: "${message.substring(0, 30)}..." → ${result.type}/${result.noteType || 'task'} (${Math.round(result.confidence * 100)}%)`);
    ...
    try {
      const response = await this.client.chat.completions.create({
        ...
        messages: [
          { 
            role: 'system', 
            content: 'Gere um título curto (max 50 caracteres) e descritivo para esta nota. Retorne APENAS o título, sem aspas ou formatação.' 
          },
...
export { AIService, ClassificationResult };
