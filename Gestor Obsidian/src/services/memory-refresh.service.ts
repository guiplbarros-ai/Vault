...
class MemoryRefreshService {
  ...
  private getTimezone(): string {
    return (process.env.MEMORY_REFRESH_TIMEZONE || 'America/Sao_Paulo').trim() || 'America/Sao_Paulo';
  }

  private getFullCrons(): string[] {
    // Back-compat: MEMORY_REFRESH_CRONS == full crons
    // default: 02:00 and 14:30 (BRT)
    return parseCronList(process.env.MEMORY_REFRESH_FULL_CRONS || process.env.MEMORY_REFRESH_CRONS || '', ['0 2 * * *', '30 14 * * *']);
  }
  ...
  private getLightCrons(): string[] {
    ...
    return parseCronList(process.env.MEMORY_REFRESH_LIGHT_CRONS || '', ['15 8 * * *', '45 16 * * *']);
  }
    ...
    for (const expr of this.getFullCrons()) {
      const job = cron.schedule(expr, async () => {
        try {
          ...
          logger.error(`Memory Refresh: erro no job (${e instanceof Error ? e.message : String(e)})`);
        }
      }, { timezone });
      this.jobs.push(job);
  ...
  async run(input: { trigger: 'scheduled' | 'manual'; mode: 'full' | 'light'; chatId?: number }): Promise<{ ok: boolean; indexedNotes: number }> {
    ...
    try {
      ...
      if (!idx.enabled()) {
        logger.warn('Memory Refresh: Supermemory index desativado (SUPERMEMORY_INDEX_SUPABASE_NOTES=0 ou SUPERMEMORY_API_KEY ausente).');
      }
      ...
      for (const workspaceId of ['pessoal', 'freelaw']) {
        ...
        if (input.mode === 'full') {
          try {
            ...
            logger.warn(`Memory Refresh: Notion ingest falhou (ws=${workspaceId}): ${e instanceof Error ? e.message : String(e)}`);
          }
        }
        ...
        try {
          ...
          await supabase.from('audit_log').insert([{
            ...
            entity_type: 'notes',
            entity_id: null,
            meta: { trigger: input.trigger, mode: input.mode, indexed_notes: batch.length, since, cursor: nextCursor },
          }]);
        } catch {
      ...
      logger.info(`Memory Refresh: done trigger=${input.trigger} started=${startedAt} finished=${finishedAt} indexedNotes=${indexedTotal}`);
      this.running = false;
    }
  }
}
