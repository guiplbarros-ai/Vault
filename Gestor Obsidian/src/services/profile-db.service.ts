...
class ProfileDbService {
  ...
  async getOrCreate(workspaceId?: string): Promise<DbProfile> {
    ...
    if (error || !data) throw new Error(`Supabase profiles insert failed: ${error?.message || 'unknown'}`);
  ...
  async setBirthday(day: number, month: number, workspaceId?: string): Promise<DbProfile> {
    ...
    if (error || !data) throw new Error(`Supabase profiles update failed: ${error?.message || 'unknown'}`);
    return data as DbProfile;
  }
}
