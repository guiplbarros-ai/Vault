...
function getWorkspaceId(workspaceId?: string): string {
  return (workspaceId || process.env.CORTEX_WORKSPACE_ID || process.env.CORTEX_DEFAULT_WORKSPACE || 'pessoal').trim();
}
...
class NotesDbService {
  ...
  async upsertExternalNote(input: {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .single();
    if (error || !data) throw new Error(`Supabase notes upsertExternalNote failed: ${error?.message || 'unknown'}`);
  ...
  async search(query: string, limit: number = 10, workspaceId?: string): Promise<DbNote[]> {
    const supabase = getSupabaseClient();
    const q = (query || '').trim();
    if (!q) return [];
   ...
   */
  async listUpdatedSince(input: { workspaceId?: string; since?: string | null; limit?: number }): Promise<DbNote[]> {
    const supabase = getSupabaseClient();
    const wid = getWorkspaceId(input.workspaceId);
    const limit = Number.isFinite(Number(input.limit)) ? Math.max(1, Math.min(2000, Number(input.limit))) : 500;
