...
export function createGmailCommand(): Command {
  const gmail = new Command('gmail')
    .alias('mail')
    .description('Gerencia emails no Gmail');
  ...
  gmail
    ...
    .action(async (options) => {
      try {
        ...
        const messageRefs = await service.getUnreadMessages(parseInt(options.max));
  ...
  gmail
    ...
    .action(async (options) => {
      try {
        const service = getGmailService();
        const messageRefs = await service.getImportantUnread(parseInt(options.max));
  ...
  gmail
    ...
    .action(async (query, options) => {
      try {
        const service = getGmailService();
        const messages = await service.search(query, parseInt(options.max));
  ...
  gmail
    ...
    .action(async (options) => {
      try {
        const service = getGmailService();
        const messageRefs = await service.getTodayMessages(parseInt(options.max));
