...
export function createPolymarketCommand(): Command {
  const polymarket = new Command('polymarket').description('Consulta mercados da Polymarket (Gamma API)');
    ...
    .action(async (query, options) => {
      try {
        ...
        markets.slice(0, limit).forEach((m, i) => {
          ...
          console.log(`${i + 1}. ${q}`);
          console.log(`   id: ${m.id}`);
          console.log(`   ${left}: ${formatPct(toProb(prices[0]))} | ${right}: ${formatPct(toProb(prices[1]))}`);
          console.log('');
        });
      } catch (error) {
  ...
  polymarket
    ...
    .action(async (id, options) => {
      try {
        ...
        const pm = getPolymarketService();
        const m = await pm.getMarketById(String(id));
        const title = String(m.question || m.title || `Polymarket ${id}`).trim().slice(0, 120);
