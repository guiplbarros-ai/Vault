...
export function createFinancasCommand(): Command {
  const financas = new Command('financas')
    .description('Calcula % Meta e Projeção Final e gera tabela Markdown');

  financas
    .command('table')
    .description('Gera tabela Markdown a partir de um JSON')
    .requiredOption('-d, --data <path>', 'Caminho para JSON com metas e realizados')
    .option('--write <relativePath>', 'Escreve/atualiza a tabela dentro de uma nota do vault (usa marcadores)')
  ...
  financas
    ...
    .description('Projeta recebimentos considerando prazos por método (PIX D+1, boleto D+5, cartão D+30)')
    .requiredOption('-d, --data <path>', 'Caminho para JSON com receivedToDate e lista de pendências')
    .action(async (options) => {
      try {
        const raw = fs.readFileSync(options.data, 'utf-8');
        ...
        console.log(`Previsto a creditar até o fim do mês: R$ ${formatNumberPtBR(res.projectedRemainingInMonth, 2)}`);
        console.log(`TOTAL projetado no mês: R$ ${formatNumberPtBR(res.projectedTotalInMonth, 2)}`);
        console.log(`Previsto para cair após o mês: R$ ${formatNumberPtBR(res.projectedAfterMonth, 2)}`);
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Erro desconhecido';
        logger.error(message);
        console.error(`✗ Erro: ${message}`);
