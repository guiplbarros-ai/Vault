import { getDaysInCurrentMonth, getDayMonthShortPtBR } from './date.js';
...
export function formatNumberPtBR(n: number, fractionDigits: number = 0): string {
...
export function formatPercentPtBR(ratio: number, fractionDigits: number = 2): string {
  return new Intl.NumberFormat('pt-BR', {
    style: 'percent',
    minimumFractionDigits: fractionDigits,
    maximumFractionDigits: fractionDigits,
...
export function projectRecebimentos(input: RecebimentosProjectionInput): RecebimentosProjectionResult {
...
export function buildFinancasMarkdownTable(input: FinanceTableInput): string {
  ...
  const rows = input.items.map((it) => {
    ...
    const projOverride = it.projecaoFinal === undefined ? undefined : parsePtBRNumber(it.projecaoFinal);
