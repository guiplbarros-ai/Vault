...
export function verifyOAuthState(state: string, secret: string, ttlMs: number = 15 * 60_000): OAuthStatePayload {
  ...
  if (!parsed?.chatId || !parsed?.workspaceId || !parsed?.ts) throw new Error('state inválido (payload)');
