...
async function ensureGoogleAccounts(chatId: number): Promise<{ workspaceId: string; accounts: string[] }> {
...
async function ensureGoogleMutatingAccount(
  ...
  if (mut.accountEmail) return { workspaceId: mut.workspaceId, account: mut.accountEmail, accounts: mut.accountEmails };
...
export function createDriveListTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      const max = maxRaw ? Math.max(1, Math.min(200, parseInt(maxRaw, 10))) : 30;
      ...
      const orderBy = firstNonEmpty((params as any).orderBy, (params as any).orderby).trim() || 'modifiedTime desc';
      ...
      const perAccount = await Promise.all(targetAccounts.map(async (acc) => {
        try {
          ...
          return { acc: labelAccount(acc), ok: true as const, count: files.length, lines: lines || '• (nenhum resultado)' };
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
          return { acc: labelAccount(acc), ok: false as const, count: 0, lines: `• ❌ erro: ${msg.slice(0, 280)}` };
        }
      }));
...
export function createDriveCreateFolderTool(): AgentTool {
  return {
    ...
    description: 'Cria uma pasta no Google Drive (ação mutável). No pool pessoal, exige conta ativa.',
    async execute(params, ctx) {
      const name = firstNonEmpty(params.name).trim();
      const parentId = firstNonEmpty((params as any).parentId, (params as any).parentid).trim() || null;
      if (!name) return 'Parâmetro obrigatório: name';

      const { workspaceId, account, accounts } = await ensureGoogleMutatingAccount(ctx.chatId, firstNonEmpty((params as any).account) || null);
      if (!accounts.length) return 'Google não autenticado. Diga "conectar google" para autorizar.';
      if (!account) {
      ...
      ctx.appendInternalData('DRIVE_CREATE_FOLDER', `account=${account}\nname=${name}\nfolderId=${folder.id}`);
...
export function createDriveRenameTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      if (before.trashed) return 'Este item está na lixeira (trashed=true). Recupere no Drive antes de renomear.';
      const after = await drive.rename(id, name);
      ctx.appendInternalData('DRIVE_RENAME', `account=${account}\nid=${id}\nfrom=${before.name}\nto=${after.name}`);
...
export function createDriveMoveTool(): AgentTool {
  return {
    name: 'DRIVE_MOVE',
    description: 'Move arquivo/pasta entre pastas. Requer id e toParentId; fromParentId é recomendado.',
    async execute(params, ctx) {
      const id = firstNonEmpty(params.id).trim();
      const toParentId = firstNonEmpty((params as any).toParentId, (params as any).toparentid).trim();
      const fromParentId = firstNonEmpty((params as any).fromParentId, (params as any).fromparentid).trim() || undefined;
      if (!id) return 'Parâmetro obrigatório: id';
      ...
      const drive = getGoogleDriveService(workspaceId, toAccountEmail(account));
      const before = await drive.getFile(id, 'id,name,mimeType,parents,trashed,webViewLink');
      if (before.trashed) return 'Este item está na lixeira (trashed=true). Recupere no Drive antes de mover.';

      const removeParents = fromParentId
        ? fromParentId
        : (before.parents && before.parents.length ? before.parents.join(',') : undefined);

      const after = await drive.move(id, { addParents: toParentId, removeParents });
      ctx.appendInternalData('DRIVE_MOVE', `account=${account}\nid=${id}\nname=${before.name}\nparents_before=${(before.parents || []).join(',')}\nparents_after=${(after.parents || []).join(',')}`);
    ...
    async execute(params, ctx) {
      ...
      for (const acc of accounts) {
        try {
          ...
          if (meta.trashed) return 'Este item está na lixeira (trashed=true). Recupere no Drive antes de ler.';
...
export function createDriveCleanEmptyFoldersTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      const maxDepth = maxDepthRaw ? Math.max(0, Math.min(10, parseInt(maxDepthRaw, 10))) : 2;
      const quarantineName = firstNonEmpty((params as any).quarantineName, (params as any).quarantinename, '🧹 _Empty Folders (quarentena)').trim();
      const trash = (firstNonEmpty((params as any).trash).toLowerCase().trim() === 'true');
      ...
      if (emptyFolders.length === 0) return `✅ Nenhuma pasta vazia encontrada (maxDepth=${maxDepth}).`;
      ...
      for (const f of emptyFolders) {
        ...
        if (quarantineId) {
          ...
          await drive.move(f.id, { addParents: quarantineId, removeParents: removeParents || undefined });
          moved += 1;
        }
      }
...
function classifyAlluFile(name0: string): { bucket: string; year?: '2023' | '2024' | '2025' } {
  ...
  if (name.includes('working model')) return { bucket: '50 - Investidores/30 - Modelos Financeiros' };
  if (name.includes('unit economics')) return { bucket: '50 - Investidores/30 - Modelos Financeiros' };
  ...
  if (name.endsWith('.zip') || name.includes('clicksign')) return { bucket: '20 - Jurídico/20 - Assinaturas (Clicksign)' };

  if (name.includes('organograma')) return { bucket: '10 - Societário/30 - Organograma & Governança' };
  if (name.includes('alteracao contratual') || name.includes('jucemg')) return { bucket: '10 - Societário/10 - Alterações Contratuais' };
  if (name.includes('cessao') || name.includes('quota') || name.includes('quotas')) return { bucket: '10 - Societário/20 - Quotas (Cessões)' };

  if (name.includes('nda')) return { bucket: '20 - Jurídico/30 - Modelos' };
  if (name.includes('contrato') || name.includes('mutuo') || name.includes('termo')) return { bucket: '20 - Jurídico/10 - Contratos' };
  ...
  if (name.includes('endividamento') || name.includes('juros') || name.includes('principal')) return { bucket: '30 - Financeiro/30 - Dívidas' };
  if (name.includes('fechamento') || name.includes('apuracao') || name.includes('apuração') || name.includes('backup sap')) return { bucket: '30 - Financeiro/40 - Fechamentos' };
  if (name.includes('simulacao') || name.includes('simulação')) return { bucket: '50 - Investidores/30 - Modelos Financeiros' };
  if (name.includes('bp ') || name.includes('bp-') || name.includes('bp -') || name.includes('financials')) return { bucket: '50 - Investidores/30 - Modelos Financeiros' };

  if (name.includes('status report')) return { bucket: '60 - Relatórios/10 - Status Reports' };
  if (name.includes('management report')) return { bucket: '60 - Relatórios/20 - Management Reports' };
  if (name.includes('report')) return { bucket: '60 - Relatórios/30 - Outros' };

  return { bucket: '00 - Inbox' };
}

async function ensureFolderByName(drive: ReturnType<typeof getGoogleDriveService>, parentId: string, folderName: string): Promise<string> {
  ...
  const existing = await drive.listFiles({ q, pageSize: 10, fields: 'files(id,name,mimeType,parents,trashed)' });
...
async function ensurePath(drive: ReturnType<typeof getGoogleDriveService>, rootId: string, pathStr: string): Promise<string> {
...
export function createDriveOrganizeAlluTool(): AgentTool {
  return {
    ...
    description: 'Organiza a pasta "Allu" (ou equivalente) criando subpastas e movendo arquivos por regras de nome.',
    async execute(params, ctx) {
      const folderId = firstNonEmpty((params as any).folderId, (params as any).folderid).trim();
      const mode = normMode(firstNonEmpty((params as any).mode));
