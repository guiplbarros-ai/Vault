...
async function ensureGoogleMutatingAccount(chatId: number, override?: string | null): Promise<{
  workspaceId: string;
  account: string | null;
  accounts: string[];
}> {
...
function calendarRangeForDay(when: string, tz: string): { timeMin: string; timeMax: string; label: string } {
...
function fmtDateTimeShort(d: Date, tz: string): string {
  ...
  // Ex: "seg., 13/01 14:30"
  return z.toLocaleString('pt-BR', { timeZone: tz, weekday: 'short', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' });
}
...
export function createCalendarTodayTool(): AgentTool {
  return {
    ...
    async execute(_params, ctx) {
      ...
      if (accounts.length === 0) return 'Google não autenticado. Diga "conectar google" para autorizar.';
      ...
      const perAccount = await Promise.all(accounts.map(async (acc) => {
        ...
        const lines = events.map(e => {
          ...
          const time = parsed.isAllDay
            ? '📅 Dia inteiro'
            : `🕐 ${fmtTime(parsed.start, tz)}`;
          const meet = parsed.meetLink ? ' 🔗' : '';
          const id = parsed.id || '';
          const link = parsed.link || '';
          const suffix = [id && `id=${id}`, link && `link=${link}`].filter(Boolean).join(' ');
          return { when: parsed.start.getTime(), line: `• [${acc}] ${time} - ${parsed.title}${meet}${suffix ? ` (${suffix})` : ''}` };
        });
        return { acc, lines };
      }));
    ...
    async execute(_params, ctx) {
      ...
      const perAccount = await Promise.all(accounts.map(async (acc) => {
        ...
        const lines = events.map(e => {
          ...
          const when = p.isAllDay ? toZonedTime(p.start, tz).toLocaleDateString('pt-BR', { timeZone: tz }) : fmtDateTimeShort(p.start, tz);
...
export function createCalendarNextTool(): AgentTool {
  return {
    ...
    async execute(_params, ctx) {
      ...
      const next = candidates.filter(Boolean).sort((a, b) => (a!.parsed.start.getTime() - b!.parsed.start.getTime()))[0];
      if (!next) return 'Nenhum próximo evento agendado.';

      const time = next.parsed.isAllDay
        ? 'Dia inteiro'
        : toZonedTime(next.parsed.start, tz).toLocaleString('pt-BR', { timeZone: tz, weekday: 'long', hour: '2-digit', minute: '2-digit' });
  ...
  return {
    ...
    async execute(_params, ctx) {
      ...
      const perAccount = await Promise.all(accounts.map(async (acc) => {
        ...
        const lines = cals.slice(0, 12).map(c => `• ${c.primary ? '⭐ ' : ''}${c.summary} (${c.id})`).join('\n');
...
export function createCalendarSearchTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      const days = params.days ? Math.max(1, Math.min(60, parseInt(params.days, 10))) : 14;
      const max = params.max ? Math.max(1, Math.min(50, parseInt(params.max, 10))) : 20;
      ...
      if (flat.length === 0) return `Nenhum evento encontrado para "${query}" nos próximos ${days} dias.`;
...
export function createCalendarInvestigateTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      const days = params.days ? Math.max(1, Math.min(60, parseInt(params.days, 10))) : 3;
      const max = params.max ? Math.max(1, Math.min(50, parseInt(params.max, 10))) : 20;
      if (!query) return 'Parâmetro obrigatório: query';
      ...
      const perAccount = await Promise.all(accounts.map(async (acc) => {
        ...
        const lines = events.map(e => {
          ...
          const whenStr = p.isAllDay
            ? toZonedTime(p.start, tz).toLocaleDateString('pt-BR', { timeZone: tz })
            : fmtDateTimeShort(p.start, tz);
          const suffix = [p.id && `id=${p.id}`, p.link && `link=${p.link}`].filter(Boolean).join(' ');
...
export function createCalendarGetEventTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      if (!parsed.account && accounts.length > 1) return `ID ambíguo no modo pessoal (pool).\nUse: email::id`;
...
export function createCalendarDayTool(): AgentTool {
  return {
    name: 'CALENDAR_DAY',
    description: 'Carrega eventos de um dia específico (when=today|tomorrow|YYYY-MM-DD). No pool, mescla e prefixa [email].',
    async execute(params, ctx) {
      const { workspaceId, accounts } = await ensureGoogleAccounts(ctx.chatId);
      if (accounts.length === 0) return 'Google não autenticado. Diga "conectar google" para autorizar.';
      const when = (params.when || params.date || 'today').trim();
      const max = params.max ? Math.max(1, Math.min(80, parseInt(params.max, 10))) : 80;
      ...
      const perAccount = await Promise.all(accounts.map(async (acc) => {
        ...
        const lines = events.map(e => {
          ...
          const link = parsed.link || '';
          const suffix = [id && `id=${id}`, link && `link=${link}`].filter(Boolean).join(' ');
          return { whenTs: parsed.start.getTime(), line: `• [${acc}] ${time} - ${parsed.title}${meet}${suffix ? ` (${suffix})` : ''}` };
        });
        return lines;
      }));
...
export function createCalendarCreateTool(): AgentTool {
  return {
    ...
    description: 'Cria evento com campos (summary/start/end/attendees/meet). Ação mutável; no pool exige conta ativa.',
    async execute(params, ctx) {
      const { workspaceId, account, accounts } = await ensureGoogleMutatingAccount(ctx.chatId, params.account || null);
      if (!accounts.length) return 'Google não autenticado. Diga "conectar google" para autorizar.';
      if (!account) return `Ação ambígua no modo pessoal (pool).\nDefina a conta ativa: [EXECUTE:GOOGLE_SET_ACCOUNT]email: <conta>[/EXECUTE]`;
      ...
      const attendees = (params.attendees || '').split(',').map(s => s.trim()).filter(Boolean);
...
export function createCalendarUpdateTool(): AgentTool {
  return {
    ...
    description: 'Atualiza um evento por id. Ação mutável; no pool use email::id ou selecione conta ativa.',
    async execute(params, ctx) {
      ...
      if (params.start) updates.start = params.start.includes('T') ? { dateTime: params.start } : { date: params.start };
      if (params.end) updates.end = params.end.includes('T') ? { dateTime: params.end } : { date: params.end };
      if (params.attendees) {
        const attendees = params.attendees.split(',').map(s => s.trim()).filter(Boolean);
        updates.attendees = attendees.map((email: string) => ({ email, responseStatus: 'needsAction' }));
      }
...
export function createCalendarDeleteTool(): AgentTool {
  return {
    ...
    description: 'Deleta um evento por id. Ação mutável; no pool use email::id ou selecione conta ativa.',
    async execute(params, ctx) {
      const raw = (params.id || params.eventId || '').trim();
      if (!raw) return 'Parâmetro obrigatório: id';
