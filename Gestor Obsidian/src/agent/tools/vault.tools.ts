...
export function createSearchVaultTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      if (!notesDb.enabled()) {
        throw new Error('Supabase não configurado. Configure SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY para buscar notas.');
      }
      ...
      ctx.appendInternalData(`FONTE: supabase.notes/${top.id}${title}${src}`, top.body_md.substring(0, 6500));
...
export function createReadNoteTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      const notesDb = getNotesDbService();
      if (!notesDb.enabled()) {
        throw new Error('Supabase não configurado. Configure SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY para ler notas.');
      }
