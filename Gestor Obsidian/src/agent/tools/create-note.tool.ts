...
import { getAreaRouterService, type WorkspaceId } from '../../services/area-router.service.js';
...
export function createCreateNoteTool(): AgentTool {
  return {
    ...
    async execute(params, ctx) {
      ...
      if (!notesDb.enabled()) {
        throw new Error('Supabase não configurado. Configure SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY para salvar notas.');
      }
