  ...
  return {
    ...
    async execute(_params, ctx) {
      ...
      const perAccount = await Promise.all(accounts.map(async (acc) => {
        ...
        const block = userLabels.map(l => `• ${l.name} (${l.id})`).join('\n') || '• (nenhuma label de usuário)';
        return `=== ${acc} ===\n${block}`;
      }));
...
async function resolveLabelId(gmail: ReturnType<typeof getGmailService>, label: string): Promise<string> {
  ...
  return {
    ...
    async execute(params, ctx) {
      ...
      for (const t of ids) {
        ...
        if (parsed.account) {
          if (!accounts.includes(parsed.account)) return `Conta inválida no modo pool: ${parsed.account}`;
...
export function createGmailSendTool(): AgentTool {
  return {
    name: 'GMAIL_SEND',
    description: 'Envia um email (ação mutável). No pool pessoal, exige conta ativa (GOOGLE_SET_ACCOUNT).',
    async execute(params, ctx) {
      ...
      await gmail.sendEmail({
        ...
        subject,
        body,
        cc: params.cc ? params.cc.split(',').map(s => s.trim()).filter(Boolean) : undefined,
        bcc: params.bcc ? params.bcc.split(',').map(s => s.trim()).filter(Boolean) : undefined,
      });
