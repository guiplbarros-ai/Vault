...
export function createGoogleListAccountsTool(): AgentTool {
  return {
    ...
    async execute(_params, ctx) {
      ...
      if (!chatDb.enabled() || !googleDb.enabled()) return 'Supabase não configurado para listar contas Google.';
      ...
      if (emails.length === 0) return `Nenhuma conta Google conectada no contexto ${workspaceId}. Diga "conectar google".`;
...
export function createGoogleSetAccountTool(): AgentTool {
  return {
    ...
    description: 'Define qual conta Google usar para ações mutáveis no contexto atual (especialmente no pool pessoal)',
    async execute(params, ctx) {
      const email = (params.email || '').toLowerCase().trim();
      const chatDb = getChatSettingsDbService();
      const googleDb = getGoogleTokensDbService();
      if (!chatDb.enabled() || !googleDb.enabled()) return 'Supabase não configurado para selecionar conta Google.';
      ...
      if (!emails.includes(email)) return `Essa conta não está conectada no contexto ${workspaceId}: ${email}\nContas: ${emails.join(', ')}`;
