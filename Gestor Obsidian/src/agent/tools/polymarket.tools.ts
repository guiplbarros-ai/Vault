...
import { getPolymarketService, type PolymarketMarket, type PolymarketPublicSearchResponse } from '../../services/polymarket.service.js';
...
function formatMarketLine(m: PolymarketMarket): string {
  const q = (m.question || '').toString().trim() || (m.title || '').toString().trim() || `market ${m.id}`;
...
function formatMarketMarkdown(m: PolymarketMarket): string {
  ...
  if (typeof m.endDate === 'string' && m.endDate.trim()) lines.push(`- endDate: ${m.endDate.trim()}`);
  if (typeof m.startDate === 'string' && m.startDate.trim()) lines.push(`- startDate: ${m.startDate.trim()}`);
  lines.push('');
...
export function createPolymarketSearchTool(): AgentTool {
  return {
    ...
    description: 'Busca mercados/eventos na Polymarket por texto e carrega amostra no contexto interno',
    async execute(params, ctx) {
      ...
      const keepClosed = String(params.keep_closed || params.include_closed || '').trim().toLowerCase() === 'true';
...
export function createPolymarketGetMarketTool(): AgentTool {
  return {
    ...
    description: 'Busca detalhes de um mercado da Polymarket por id (salva detalhes no contexto interno)',
    async execute(params, ctx) {
      const pm = getPolymarketService();
...
export function createPolymarketCaptureMarketTool(): AgentTool {
  return {
    ...
    description: 'Captura snapshot de um mercado e salva como nota (Supabase) e/ou memória (Supermemory)',
    async execute(params, ctx) {
      const pm = getPolymarketService();
      const id = (params.id || '').toString().trim();
      if (!id) return 'Parâmetro obrigatório: id';
      ...
      const title = ((m.question || m.title || '').toString().trim() || `Polymarket ${id}`).slice(0, 120);
      ...
      if (notesDb.enabled()) {
        const saved = await notesDb.upsertExternalNote({
          ...
          tags: [
            ...
            `polymarket/id/${id}`,
            ...(typeof m.slug === 'string' && m.slug.trim() ? [`polymarket/slug/${m.slug.trim()}`] : []),
          ],
          context: 'unknown',
        });
