 ...
 */
export async function resolveGoogleMutatingAccount(chatId: number): Promise<GoogleMutatingAccountContext> {
  const chatDb = getChatSettingsDbService();
  const googleDb = getGoogleTokensDbService();

  if (!chatDb.enabled() || !googleDb.enabled()) {
    return { workspaceId: 'pessoal', accountEmails: [], selectedAccountEmail: null, accountEmail: null };
  }
  ...
  if (workspaceId === 'freelaw') {
    const acc = (selectedAccountEmail && accountEmails.includes(selectedAccountEmail))
  ...
  if (accountEmails.length <= 1) {
    return { workspaceId, accountEmails, selectedAccountEmail, accountEmail: accountEmails[0] || null };
  }
